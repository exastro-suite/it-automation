...
if ! test -e "$SQL_DIR/drop_db_and_user_for_MySQL.sql" ; then
    ...
    DB_NAME_CHK=`env MYSQL_USER="root" MYSQL_PWD="$DB_ROOT_PASSWORD_ON_CMD" mysql -e "SHOW DATABASES LIKE '$DB_NAME';" 2>> "$LOG_FILE"`
    DB_USERNAME_CHK=`env MYSQL_USER="root" MYSQL_PWD="$DB_ROOT_PASSWORD_ON_CMD" mysql -e "SELECT user, host FROM mysql.user where user = '$DB_USERNAME';" 2>> "$LOG_FILE"`

    if [ -z "$DB_NAME_CHK" -o -z "$DB_USERNAME_CHK" ] ; then
        log 'ERROR : Should be set correct [db_root_password][db_name][db_username].'
    else
        env MYSQL_USER="root" MYSQL_PWD="$DB_ROOT_PASSWORD_ON_CMD" mysql < /tmp/drop_db_and_user_for_MySQL.sql 2>> "$LOG_FILE"
    fi
