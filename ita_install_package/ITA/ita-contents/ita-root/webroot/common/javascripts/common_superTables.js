...
/////* ソート機能と融合させたいのでとりあえずfixedColsオプションは禁止 */
	if (this.fixedCols > 0)
	{
		alert("fixedColsオプションは使用禁止(sort機能と融合のため)");
		return -1;
	}
...
/////* Create the framework dom */
	...
	//
	this.sDataInner = this.sBase.cloneNode(false);
	//
	...
	this.sDataInner.className = "sDataInner";
...
/////* Set up the colGroup */
	...
	for (i=0, j=this.colWidths.length; i<j; i++) {
		...
		this.sColGroup.lastChild.setAttribute("class", tableId + i);
	}
	...
	if (this.cssSkin !== "") {
		...
		this.sDataTable.id = tableId + "-Main";
		this.sHeaderTable.id = tableId + "-Headers";
	...
	/////* DataTableにIDを「元TableID＋"_data"」としたtbodyタグを追加するよう改造 */	
	var target = this.sDataTable.getElementsByTagName("TBODY");
	target[0].setAttribute("id", tableId + "_data");
	...
	this.sData.appendChild(this.sDataInner);
	this.sDataInner.appendChild(this.sDataTable);
