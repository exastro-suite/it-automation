...
// ----サイト個別メニュー、ここから
    ...
    if ( execution_no.length > 0 ){
        ...
        if( execution_no.match( /^[-]?[0-9]+(\.[0-9]+)?$/ ) ){
            // フィルタフォームに作業№をセット
            document.getElementById('filter_EXECUTION_NO').value = execution_no;
            
            // サブメニュー(表示フィルタ)を閉じる
            // show('KensakuJokenMidashi', 'KensakuJokenNakami' );
...
function monitor_execution( execution_no ){
    // 遷移先URLを作成
    var url = '/default/menu/01_browse.php?no=2100020313&execution_no=' + execution_no;
