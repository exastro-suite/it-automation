...
require_once ( $root_dir_path . '/libs/webindividuallibs/systems/2100160103/web_parts_for_create_master.php');
?>
...
$(function() {
    ...
    $('#submit').on('click', function() {
        if($('#create_menu_form ._create_:checkbox:checked').length > 0){
            if(window.confirm("<?php echo $g['objMTS']->getSomeMessage("ITACREPAR-MNU-103510"); ?>")){
                return true;
            }else{
                return false;
            }
        }else{
            window.alert("<?php echo $g['objMTS']->getSomeMessage("ITACREPAR-MNU-103504"); ?>");
            return false;
        }
    });
});
...
<div id="SetsumeiNakami" style="display:block" class="text">
    <div style="margin-left:10px">
        <?php echo $g['objMTS']->getSomeMessage("ITACREPAR-MNU-103501");?>    
    </div>
</div>
<!-------------------------------- 説明 -------------------------------->
<!-------------------------------- メニュー作成-------------------------------->
...
<h2>
    <table width="100%">
        <tr>
            <td><div onClick=location.href="javascript:show('Mix1_Midashi','Mix1_Nakami');" class="midashi_class" ><?php echo $g['objMTS']->getSomeMessage("ITACREPAR-MNU-103502"); ?></div></td>
            <td>
...
<?php endforeach; ?>
            ...
            <input type="submit" id="submit" value="<?php echo $g['objMTS']->getSomeMessage("ITACREPAR-MNU-103503"); ?>">
