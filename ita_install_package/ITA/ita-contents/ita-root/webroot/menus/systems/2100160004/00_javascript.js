...
function setFilter_special(){
    ...
    var startObj = document.getElementById('start_no');
    var endObj = document.getElementById('end_no');
    var start_no = startObj.value;
    var end_no = endObj.value;
    var initialFilterEl;
    var initialFilter;

    if( start_no.length > 0 ){
            var objTgtNum = document.getElementById('Filter1Tbl_1__S');
            objTgtNum.value = start_no;
            var objTgtNum = document.getElementById('Filter1Tbl_1__E');
            objTgtNum.value = start_no;
    }
    if( end_no.length > 0 ){
            var objTgtNum = document.getElementById('Filter1Tbl_1__E');
            objTgtNum.value = end_no;
    }
    //フィルターボタンを押した処理
    if( start_no.length > 0 ){
...
function jumpToCreateMenu(file_name){

    var decFileName = decodeURIComponent(escape(atob(file_name)));

    if(decFileName != ""){

        menu_id = decFileName.substr(0,10);
        // 遷移先URLを作成
        var url = '/default/menu/01_browse.php?no=' + encodeURI(menu_id);

        // 作業状態確認メニューに遷移
        window.open(url);
    }
    else{
        window.alert(getSomeMessage("ITACREPAR_1001"));
    }
}
