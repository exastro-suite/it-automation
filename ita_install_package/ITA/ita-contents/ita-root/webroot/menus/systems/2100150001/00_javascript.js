...
callback.prototype = {  
    ...
    initial_sync : function( result ){
        ...
        enableButton(ary_result[2]);

        if( ary_result[0] == "000" ){
            window.alert(getSomeMessage("ITAMATERIAL_1004"));
        }else{
...
function initial_sync( remortRepoUrl, branch, cloneRepoDir, password, objButton){
    ...
    if( "" == decRemortRepoUrl || "" == decCloneRepoDir ){
        window.alert(getSomeMessage("ITAMATERIAL_1001"));
    }
    else if( window.confirm(getSomeMessage("ITAMATERIAL_1002"))){
