...
//
    ...
    if(array_key_exists("privilege", $g) && $g['privilege'] === '2'){
        ...
        $allDumpMsg = $g['objMTS']->getSomeMessage("ITAWDCH-STD-30062");
    }else{
        $boolShowRegisterArea = true;
        if($table->getFormatter("register_table")->getGeneValue("hidden")===true){
            $boolShowRegisterArea = false;
        }
    }
    //----メッセージtmpl作成準備
    $aryImportFilePath[] = $objMTS->getTemplateFilePath("ITAWDCC","STD","_js");
    $aryImportFilePath[] = $objMTS->getTemplateFilePath("ITAMATERIAL","STD","_js");
