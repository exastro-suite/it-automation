...
callback.prototype = {  
    ...
    ethWakeOrderSend : function(result){
        var strMixOuterFrameName = 'Mix1_Nakami';
        var strMixInnerFramePrefix = 'Mix1_';
        ...
        if( ary_result[0] == "000" ){
            Mix1_1_ethWakeOrderSend(null,null,null,ary_result);
        }else if( ary_result[0] == "002" ){
            window.alert(getSomeMessage("ITAWDCC90102"));
            objAlertArea.innerHTML = resultContentTag;
            objAlertArea.style.display = "block";
            buttonDisableControl('off', ary_result[4]);
        }else if( ary_result[0] == "003" ){
            var objUpdateArea=$('#'+strMixOuterFrameName+' .table_area').get()[0];
            objUpdateArea.innerHTML="";
            objAlertArea.innerHTML = resultContentTag;
...
function Mix1_1_ethWakeOrderSend(objButton,tgtSystemId,strLT4U,ary_result){
    //$(objButton).attr("disabled",true);

    if( objButton === null 
        && tgtSystemId === null 
        && ary_result instanceof Array){
        //----活性化待ちループの場合
        if( ary_result[1] == "001" ){
            // ブラウザ側で待つと固まるので、サーバーへプロセスを再度渡す
            alert(ary_result[2]);
            strTid = ary_result[4];
            proxy.ethWakeOrderSend('2', strTid, "", "");
        }
        else if( ary_result[1] == "002" ){
            // ボタンを再活性化
            var retStrTid =  buttonDisableControl('off', ary_result[4]);
        }
        //活性化待ちループの場合----
    }
    else{

        if( window.confirm( getSomeMessage("ITABASEC030101",{0:tgtSystemId}) ) ){
            //----ボタンを非活性にして、サーバーへWOL送信をリクエスト
            var retStrTid =  buttonDisableControl('on', null, objButton);
            if( retStrTid !== null ){
                proxy.ethWakeOrderSend('1', retStrTid, tgtSystemId ,strLT4U);
            }
            //ボタンを非活性にして、サーバーへWOL送信をリクエスト----
        }
    }
