...
callback.prototype = {  
    ...
    ,
    dispUserListHeader : function(result){
        var ary_result = getArrayBySafeSeparator(result);
        checkTypicalFlagInHADACResult(ary_result);
        if ( result == "unexpected_error" ){
            window.alert(getSomeMessage("ITAWDCC90101"));
        }else{
            document.getElementById('UserBrowse_header_area').innerHTML = result;
    ...
    dispUserList : function(result){
        ...
        if ( result == "unexpected_error" ){
            ...
            document.getElementById('UserBrowse_area').innerHTML = result;
            adjustTableAuto ( "DbTable_Yobi1",
                            "sDefault",
                            "fakeContainer_Yobi1",
                            600,
                            900 );
            jumpToSelfHtml('UserBrowseMidashi');
            
        }
    },
    dispMenuListHeader : function(result){
        var ary_result = getArrayBySafeSeparator(result);
        checkTypicalFlagInHADACResult(ary_result);
        if ( result == "unexpected_error" ){
            window.alert(getSomeMessage("ITAWDCC90101"));
        }else{
            document.getElementById('MenuBrowse_header_area').innerHTML = result;
            adjustTableAuto ( "DbTable_Yobi2",
                            "sDefault",
                            "fakeContainer_Yobi2",
                            600,
                            900 );
    ...
    dispMenuList : function(result){
        ...
        if ( result == "unexpected_error" ){
            ...
            document.getElementById('MenuBrowse_area').innerHTML = result;
            adjustTableAuto ( "DbTable_Yobi3",
                            "sDefault",
                            "fakeContainer_Yobi3",
                            600,
                            900 );
            jumpToSelfHtml('MenuBrowseMidashi');
...
// ----サイト個別メニュー、ここから
    show('UserBrowseMidashi'    ,'UserBrowseNakami'     );
    show('MenuBrowseMidashi'    ,'MenuBrowseNakami'     );
    document.getElementById('UserBrowse_area').innerHTML = getSomeMessage("ITAWDCC50104");
    document.getElementById('MenuBrowse_area').innerHTML = getSomeMessage("ITAWDCC50105");
// サイト個別メニュー、ここまで----
...
function Journal1Tbl_search_async(){
    //alert('wake-fx(Journal1Tbl_search_async)');
    // 履歴検索実施フラグを初期化
    var journal1AreaWrap = 'Journal1_Nakami';
...
function Mix1_1_disp_user_list( role_id ){
    // 変数を定義
    var p_role_id                   = role_id;

    // 表示用領域を開く
    if( checkOpenNow("UserBrowseNakami")===false ){
        show('UserBrowseMidashi','UserBrowseNakami');
    }

    // しばらくお待ち下さいを出す
    document.getElementById('UserBrowse_area').innerHTML = "<div id=\"wait_msg\" >"+getSomeMessage("ITAWDCC10102")+"</div>";

    //proxy.dispUserListHeader実行
    proxy.dispUserListHeader( p_role_id );

    //proxy.dispUserList実行
    proxy.dispUserList( p_role_id );
}

//////// メニュー情報表示用ファンクション ////////
function Mix1_1_disp_menu_list( role_id ){
    // 変数を定義
    var p_role_id                   = role_id;

    // 表示用領域を開く
    if( checkOpenNow("MenuBrowseNakami")===false ){
        show('MenuBrowseMidashi','MenuBrowseNakami');
    }

    // しばらくお待ち下さいを出す
    document.getElementById('MenuBrowse_area').innerHTML = "<div id=\"wait_msg\" >"+getSomeMessage("ITAWDCC10102")+"</div>";

    //proxy.dispMenuListHeader実行
    proxy.dispMenuListHeader( p_role_id );

    //proxy.dispMenuList実行
    proxy.dispMenuList( p_role_id );
}
