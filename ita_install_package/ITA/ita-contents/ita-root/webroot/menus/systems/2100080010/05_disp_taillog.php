...
//
    ...
    try{
        ...
        if( $objQuery->getStatus()===false ){
            ...
            // 例外処理へ
            throw new Exception( $objMTS->getSomeMessage("ITATERRAFORM-ERR-501",array($strIfTableIdForSelect)) );
        }
        ...
        if (!$r){
            ...
            // 例外処理へ
            throw new Exception( $objMTS->getSomeMessage("ITATERRAFORM-ERR-502",array($strIfTableIdForSelect)) );
        }
        ...
        if( $num_of_rows != 1 ){
            ...
            // 例外処理へ
            throw new Exception( $objMTS->getSomeMessage("ITATERRAFORM-ERR-503",array($strIfTableIdForSelect)) );
        }
        ...
        if (isset($_GET['load'])){
            ...
            if( !array_key_exists( "execution_no", $_GET ) ){
                ...
                // 例外処理へ
                throw new Exception( $objMTS->getSomeMessage("ITATERRAFORM-ERR-504") );
            }
            else{
                ...
                if( $objIntNumVali->isValid($execution_no) === false ){
                    ...
                    // 例外処理へ
                    throw new Exception( $objMTS->getSomeMessage("ITATERRAFORM-ERR-505") );
                }
                unset($objIntNumVali);
            }
            ...
            if( !array_key_exists( "prg_recorder", $_GET ) ){
                ...
                // 例外処理へ
                throw new Exception( $objMTS->getSomeMessage("ITATERRAFORM-ERR-506") );
            }
            else{
                ...
                if( $objIntNumVali->isValid($prg_record_file_id) === false ){
                    ...
                    // 例外処理へ
                    throw new Exception( $objMTS->getSomeMessage("ITATERRAFORM-ERR-507") );
                }
                unset($objIntNumVali);
                
                if( array_key_exists($prg_record_file_id,$prg_recorder_array)===false ){
                    // 例外処理へ
                    throw new Exception( $objMTS->getSomeMessage("ITATERRAFORM-ERR-510") );
                }
            ...
            if ( !file_exists($prg_record_file_name_fullpath) || is_dir( $prg_record_file_name_fullpath ) ){
                ...
                throw new Exception( '<div id="tail_show" style="display:none;"></div>'.$objMTS->getSomeMessage("ITATERRAFORM-ERR-511") );
            }
            ...
            if( !empty($_GET['filter_string']) ){
                ...
                if( $strMatchLineOnly == "on" ){
                    ...
                    if(!file_exists($temp_file_name_fullpath_2)){
                        // 例外処理へ
                        throw new Exception( $objMTS->getSomeMessage("ITATERRAFORM-ERR-502",array($command_string)) );
                    }
                }
                else{
        ...
        <body>
            <div id="iframeInDispTail">
                <table border="0">
                    <tr>
                        <!--//フィルタ-->
                        <td style="padding-right:10px">{$objMTS->getSomeMessage("ITATERRAFORM-MNU-104710")}：</td>
                        <td>
                            <input onkeydown="pre_filter(event.keyCode)" type="text" id="filter_string" name="filter_string" size="20" maxlength="256">
                        </td>
                        <td style="padding-right:10px">
                            <!--//該当行のみ表示-->
                            <input type="checkbox" id="match_line_only" name="match_line_only" value="on" onClick="pre_filter('13')" >{$objMTS->getSomeMessage("ITATERRAFORM-MNU-104720")}
                        </td>
                    </tr>
                </table>
                <br>
