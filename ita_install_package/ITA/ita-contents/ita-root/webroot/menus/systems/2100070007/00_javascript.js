...
function queryDataToFilter(){
    ...
    // execution_noが取得された場合
    if ( execution_no.length > 0 ){
            // フィルタフォームにexecution_noをセット
            document.getElementById('Filter1Tbl_2__S').value = execution_no;
            document.getElementById('Filter1Tbl_2__E').value = execution_no;
            filter_flag = true;
    }
    if(filter_flag === true){
            // サブメニュー(表示フィルタ)を閉じる
