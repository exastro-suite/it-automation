...
//
    ...
    $strExeTableIdForSelect    = 'C_ANSIBLE_LRL_EXE_INS_MNG';
    ...
    try{
        ...
        if( !array_key_exists( "execution_no", $_GET ) ){
            // アクセスログ出力(クエリ無し警告)
            web_log( $objMTS->getSomeMessage("ITAANSIBLEH-ERR-103010",__FILE__.':'.__LINE__) );
        }
        ...
        if( array_key_exists( "execution_no", $_GET ) === true ){
            ...
            if( $objIntNumVali->isValid($execution_no) === false ){
                // アクセスログ出力(想定外エラー)
                web_log( $objMTS->getSomeMessage("ITAANSIBLEH-ERR-103020",__FILE__.':'.__LINE__) );
            }
        }
...
EOD;
    ...
    $strPageExplainBody = $objMTS->getSomeMessage("ITAANSIBLEH-MNU-1505090");
    /*
    $strPageExplainBody = <<< EOD
    登録したAnsible(Legacy(NS))設定作業をモニターするメニューです。<br>
作業の進行状況をリアルタイムで確認できるほか、緊急停止が可能です。 
EOD;
    */
print <<< EOD
    <!-------------------------------- 説明 -------------------------------->
    ...
    <h2>
        <table width="100%">
            <tr>
                <!--説明-->
                <td><div onClick=location.href="javascript:show('SetsumeiMidashi','SetsumeiNakami');" class="midashi_class" >{$objMTS->getSomeMessage("ITAANSIBLEH-MNU-1505020")}</div></td>
                <td>
    ...
    <h2>
        <table width="100%">
            <tr>
                <!--対象作業-->
                <td><div onClick=location.href="javascript:show('ExecutionMidashi','ExecutionNakami');" class="midashi_class" >{$objMTS->getSomeMessage("ITAANSIBLEH-MNU-1505030")}</div></td>
                <td>
...
print <<< EOD
                ...
                <h2>
                    <table width="100%">
                        <tr>
                            <!--予約取消-->
                            <td><div onClick=location.href="javascript:show('Booking_Cancel_Midashi','Booking_Cancel_Nakami');" class="midashi_class" >{$objMTS->getSomeMessage("ITAANSIBLEH-MNU-1505040")}</div></td>
                            <td>
                ...
                <div id="Booking_Cancel_Nakami" style="display:block" class="text">
                    <div style="margin-left:10px">
                        <!--予約取消-->
                        <input type="button" id="book_cancel_button" class="updatebutton" value="{$objMTS->getSomeMessage("ITAANSIBLEH-MNU-1505050")}" onClick=location.href="javascript:book_cancel();" >
                    </div>
                </div>
...
print <<< EOD
                ...
                <h2>
                    <table width="100%">
                        <tr>
                            <!--進行状況-->
                            <td><div onClick=location.href="javascript:show('Monitor_{$loop_counter}_Midashi','Monitor_{$loop_counter}_Nakami');" class="midashi_class" >{$objMTS->getSomeMessage("ITAANSIBLEH-MNU-1505060")}({$prg_recorder['PRG_RCDR_NAME']})</div></td>
                            <td>
...
print <<< EOD
    ...
    <h2>
        <table width="100%">
            <tr>
                <!--緊急停止-->
                <td><div onClick=location.href="javascript:show('ScramMidashi','ScramNakami');" class="midashi_class" >{$objMTS->getSomeMessage("ITAANSIBLEH-MNU-1505070")}</div></td>
                <td>
    ...
    <div id="ScramNakami" style="display:block" class="text">
        <div style="margin-left:10px">
            <div id="scram_area">
            <!--緊急停止-->
            <input type="button" id="scrumTryExecute" class="updatebutton scrumSubmitElement tryExecute" value="{$objMTS->getSomeMessage("ITAANSIBLEH-MNU-1505080")}" onClick=location.href="javascript:scrum_execution();" disabled >
            </div>
        </div>
    </div>
