...
function scrum_execution(){
    // '緊急停止してよろしいですか？'
    if( window.confirm( getSomeMessage("ITADSCC103030")) ){
        // ボタンを非活性にする
        document.getElementById('scrumTryExecute').disabled = true;
...
function book_cancel(){
    //'予約を取り消してよろしいですか？'
    if( window.confirm(getSomeMessage("ITADSCC103040")) ){
        // ボタンを非活性にする
        document.getElementById('book_cancel_button').disabled = true;
...
window.onload = function(){
    ...
    if ( execution_no.length == 0 ){
        ...
        window.alert(getSomeMessage("ITADSCC103050"));
        
        // 遷移先URLを作成
        var url = '/default/menu/01_browse.php?no=2100060011';
