...
callback.prototype = {  
    ...
    ,
    dispRoleListHeader : function(result){
        var ary_result = getArrayBySafeSeparator(result);
        checkTypicalFlagInHADACResult(ary_result);
        if ( result == "unexpected_error" ){
            window.alert(getSomeMessage("ITAWDCC90101"));
        }else{
            document.getElementById('RoleBrowse_header_area').innerHTML = result;
    ...
    editRoleList : function(result){
        ...
        if ( result == "unexpected_error" ){
            ...
            if( !result.match(/[^0-9]+/) ){
                // メンテナンスが成功した場合はresultにユーザID(正数値)が返却される
                //----メンテナンスしました。
                window.alert(getSomeMessage("ITAWDCC80201"));
                
                document.getElementById('RoleBrowse_area').innerHTML = '';
                edit_role_list(0,result);
            }else{
                // 正常、準正常の場合はresultをareaに流し込んでsuper_tableする
                // 正常  ：メンテナンスフォームが返ってきた場合(tableが含まれる)
                // 準正常：ユーザアカウントが廃止されている場合、別セッションからメンテナンスされている場合(排他エラー)
                document.getElementById('RoleBrowse_area').innerHTML = result;
                adjustTableAuto ( "DbTable_Yobi1",
                                "sDefault",
                                "fakeContainer_Yobi1",
                                webStdTableHeight,
                                webStdTableWidth );
                // RoleBrowseMidashiのところまでジャンプ
                jumpToSelfHtml('RoleBrowseMidashi');
            }
        }
        showForDeveloper(result);
    },
    lockRemove : function( result ){
        var ary_result = getArrayBySafeSeparator(result);
        checkTypicalFlagInHADACResult(ary_result);
...
// ----サイト個別メニュー、ここから
    show('RoleBrowseMidashi'    ,'RoleBrowseNakami'     );
    document.getElementById('RoleBrowse_area').innerHTML = getSomeMessage("ITAWDCC50106");
// サイト個別メニュー、ここまで----
...
function Mix1_1_lock_remove( user_id ){
    // ファンクション呼び出し要否フラグ
    var exec_flag = false;
    if( window.confirm(getSomeMessage("ITAWDCC70104")) ){
        exec_flag = true;
    }

    if( exec_flag ){
        proxy.lockRemove( user_id );
    }
}

function edit_role_list( mode, user_id ){
    Mix1_1_edit_role_list( mode, user_id );
}

function Mix1_1_edit_role_list( mode, user_id ){
    // 変数を定義
    var p_user_id                   = user_id;
    var p_role_array                = [];
    var p_max_last_update_timestamp = "";
    ...
    switch( mode ){
        // 参照表示(mode=0)
        case 0 :
            if ( document.getElementById('now_on_maintenance') != null ){
                //'ロール情報を編集中ですが、\n編集を破棄してよろしいですか？'
                if( !window.confirm(getSomeMessage("ITAWDCC70102")) ){
                    exec_flag = false;
                }
            }
            if ( exec_flag ){
                // 表示用領域を開く
                if( checkOpenNow("RoleBrowseNakami")===false ){
                    show('RoleBrowseMidashi','RoleBrowseNakami');
                }
                // しばらくお待ち下さいを出す
                document.getElementById('RoleBrowse_area').innerHTML = "<div id=\"wait_msg\" >"+getSomeMessage("ITAWDCC10102")+"</div>";
            }
            break;
        case 1 :
            // 編集画面に遷移(mode=1)
            // 引数準備必要なし
            setInputButtonDisable('RoleBrowseNakami','linkbutton',true);
            break;
        case 2 :
            // 編集画面で実行を押下(mode=2)
            if( window.confirm(getSomeMessage("ITAWDCC70101")) ){
                // 引数を準備
                p_max_last_update_timestamp = document.getElementById('max_last_update_timestamp').innerHTML;

                i=1;
                j=0;
                while( document.getElementById("role_id_"+i) != null ){
                    if(document.getElementById("role_id_"+i).checked){
                        p_role_array[j] = document.getElementById("role_id_"+i).value;
                        j++;
                    }
                    i++;
                }
                // チェックが0件の場合に本当に登録してOKか伺う
                if( j == 0 ){
                    //----ロールが1件も選択されていませんが反映してよろしいですか？
                    if( !window.confirm(getSomeMessage("ITAWDCC70103")) ){
                        exec_flag = false;
                    }
                }
                if( exec_flag ){
                    setInputButtonDisable('RoleBrowseNakami','updatebutton',true);
                }
            }else{
                exec_flag = false;
            }
    ...
    if( exec_flag ){
        //proxy.dispRoleListHeader実行
        proxy.dispRoleListHeader( p_user_id );
        
        //proxy.editRoleList実行
        proxy.editRoleList(mode,
                           p_user_id,
                           p_role_array,
