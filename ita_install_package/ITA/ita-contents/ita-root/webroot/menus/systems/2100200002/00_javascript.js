...
callback.prototype = {
    ...
    destroyWorkspaceInsRegister : function (result) {
        ...
        if (result[0] == true) {
            ...
            //削除失敗通知メッセージ
            window.alert(getSomeMessage("ITATERRAFORMCLI100031"));
            Filter1Tbl_search_async();
        }
    }
...
function destroyWorkspaceInsRegister(obj, workspaceID, workspaceName) {
    ...
    //エンコード
    encWorkspaceID = encodeURIComponent(workspaceID);
    encWorkspaceName = encodeURIComponent(workspaceName);
    var data = {
        'workspaceID': encWorkspaceID,
        'workspaceName': encWorkspaceName
    };

    let decordedWorkspaceName = decodeURIComponent(escape(atob(workspaceName)));
    //destroy実行確認メッセージ
    if (window.confirm(getSomeMessage("ITATERRAFORMCLI100030", decordedWorkspaceName))) {
...
// Movement一覧へのリンク
function newOpenWindow(currentText){
    if(isNaN(currentText)){
        var searchText = escape(atob(currentText));
    }else{
        var searchText = currentText;
    }
  var url = "01_browse.php?no=2100200003&filter=on&Filter1Tbl_5=" + searchText;
