...
callback.prototype = {  
    ...
    editRoleList : function(result){
        ...
        if ( result == "unexpected_error" ){
            ...
            if( !result.match(/[^0-9]+/) ){
                // メンテナンスが成功した場合はresultにメニューID(正数値)が返却される
                //----メンテナンスしました。
                ...
                // 準正常：メニューが廃止されている場合、別セッションからメンテナンスされている場合(排他エラー)
...
// ----サイト個別メニュー、ここから
    ...
    document.getElementById('RoleBrowse_area').innerHTML = getSomeMessage("ITAWDCC50102");
// サイト個別メニュー、ここまで----
...
//////// ロール情報編集用ファンクション[ラッパー] ////////
function Mix1_1_edit_role_list( mode, menu_id ){
    edit_role_list( mode, menu_id );
}

//////// ロール情報編集用ファンクション ////////
function edit_role_list( mode, menu_id ){
    // 変数を定義
    var p_menu_id                   = menu_id;
    var p_role_array_id             = [];
    var p_role_array_priv           = [];
    ...
    switch( mode ){
        ...
        case 2 :
            // 編集画面で実行を押下(mode=2)
            //----if( window.confirm('メンテナンスを実行してよろしいですか？') ){
            if( window.confirm(getSomeMessage("ITAWDCC70101")) ){
                ...
                while( document.getElementById("role_id_"+i) != null ){
                    // ロールIDの一覧をarrayに投入
                    p_role_array_id[i] = document.getElementById("role_id_"+i).innerHTML;

                    // 権限の一覧をarrayに投入
                    p_role_array_priv[i] = document.getElementById("priv_"+i).value;

                    if( p_role_array_priv[i] != 0 ){
                        j=1;
                    }
    ...
    if( exec_flag ){
        ...
        proxy.dispRoleListHeader( p_menu_id );
