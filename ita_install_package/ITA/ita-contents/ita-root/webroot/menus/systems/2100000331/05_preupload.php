...
if (preg_match('/^[a-zA-Z0-9ぁ-んァ-ヶ一-龠々._]+$/', $fileName)) {
    $filePath = $root_dir_path . "/uploadfiles/{$menuId}/{$fileName}";
    // 結果ファイルの場合、.logで後方一致
    if (preg_match("/^[a-zA-Z0-9ぁ-んァ-ヶ一-龠々._]+.log$/",$fileName)) {
        $filePath = $root_dir_path . "/uploadfiles/{$menuId}/FILE_RESULT/{$fileName}";
    }
