...
//
    ...
    $strPageInfo = $objMTS->getSomeMessage('ITABASEH-MNU-2100000331_1');

    //-- サイト個別PHP要素、ここから--
    require_once $root_dir_path . '/libs/webindividuallibs/systems/2100000213/web_parts_for_data_import_maintenance.php';
    $taskNo = '';
    if (isset($_SESSION['data_import_task_no']) === true && strlen($_SESSION['data_import_task_no']) > 0) {
        $taskNo = $_SESSION['data_import_task_no'];
    }
    //-- サイト個別PHP要素、ここまで--
...
<<< EOD
    ...
    <div id="taskNo" style="display:none" class="text">{$taskNo}</div>
    <!-------------------------------- ユーザ・コンテンツ情報 -------------------------------->
{$strDeveloperArea}
    <!-------------------------------- 説明 -------------------------------->
    <h2>
