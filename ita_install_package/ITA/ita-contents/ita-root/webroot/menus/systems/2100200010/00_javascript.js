...
function scrum_execution(){
    // '緊急停止してよろしいですか？'
    if( window.confirm( getSomeMessage("ITATERRAFORMCLI110001")) ){
        clearInterval(timerID);
        // ボタンを非活性にする
        document.getElementById('scrumTryExecute').disabled = true;
...
function book_cancel(){
    //'予約を取り消してよろしいですか？'
    if( window.confirm(getSomeMessage("ITATERRAFORMCLI110002")) ){
        // ボタンを非活性にする
        document.getElementById('book_cancel_button').disabled = true;
...
window.onload = function(){
    ...
    if ( execution_no.length == 0 ){
        ...
        window.alert(getSomeMessage("ITATERRAFORMCLI110003"));

        // 遷移先URLを作成
        var url = '/default/menu/01_browse.php?no=2100200011';
