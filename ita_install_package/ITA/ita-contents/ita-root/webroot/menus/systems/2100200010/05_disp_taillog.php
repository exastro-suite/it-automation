...
//
    ...
    try{
        ...
        if( $objQuery->getStatus()===false ){
            ...
            // 例外処理へ
            throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-501",array($strIfTableIdForSelect)) );
        }
        ...
        if (!$r){
            ...
            // 例外処理へ
            throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-502",array($strIfTableIdForSelect)) );
        }
        ...
        if( $num_of_rows != 1 ){
            ...
            // 例外処理へ
            throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-503",array($strIfTableIdForSelect)) );
        }
        ...
        if (isset($_GET['load'])){
            ...
            if( !array_key_exists( "execution_no", $_GET ) ){
                ...
                // 例外処理へ
                throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-504") );
            }
            else{
                ...
                if( $objIntNumVali->isValid($execution_no) === false ){
                    ...
                    // 例外処理へ
                    throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-505") );
                }
                unset($objIntNumVali);
            }
            ...
            if( !array_key_exists( "prg_recorder", $_GET ) ){
                ...
                // 例外処理へ
                throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-506") );
            }
            else{
                ...
                if( $objIntNumVali->isValid($prg_record_file_id) === false ){
                    ...
                    // 例外処理へ
                    throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-507") );
                }
                unset($objIntNumVali);

                if( array_key_exists($prg_record_file_id,$prg_recorder_array)===false ){
                    // 例外処理へ
                    throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-510") );
                }
            ...
            // tail対象のファイルパスを作成                               //
            ////////////////////////////////////////////////////////////////
            $terraform_data_dir = "/logs/terraform_cli_out_logs";
            $data_type = "out";
            ...
            if ( !file_exists($prg_record_file_name_fullpath) || is_dir( $prg_record_file_name_fullpath ) ){
                ...
                throw new Exception( '<div id="tail_show" style="display:none;"></div>'.$objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-511") );
            }
            ...
            if( !empty($_GET['filter_string']) ){
                ...
                if( $strMatchLineOnly == "on" ){
                    ...
                    if(!file_exists($temp_file_name_fullpath_2)){
                        // 例外処理へ
                        throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-502",array($command_string)) );
                    }
                }
                else{
        ...
        <body>
            <div id="iframeInDispTail">
                <table border="0">
                    <tr>
                        <!--//フィルタ-->
                        <td style="padding-right:10px">{$objMTS->getSomeMessage("ITATERRAFORMCLI-MNU-160010")}：</td>
                        <td>
                            <input onkeydown="pre_filter(event.keyCode)" type="text" id="filter_string" name="filter_string" size="20" maxlength="256">
                        </td>
                        <td style="padding-right:10px">
                            <!--//該当行のみ表示-->
                            <input type="checkbox" id="match_line_only" name="match_line_only" value="on" onClick="pre_filter('13')" >{$objMTS->getSomeMessage("ITATERRAFORMCLI-MNU-160020")}
                        </td>
                    </tr>
                </table>
                <br>
    ...
    function getExecuteStatus($execution_no) {
        ...
        try {
            ...
            if( $objQuery->getStatus()===false ){
                ...
                $error_msg = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-501",array($strExeTableIdForSelect));
                // ログ出力
                web_log($error_msg);
            ...
            if (!$r){
                ...
                $error_msg =$objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-502",array($strExeTableIdForSelect));
                // ログ出力
                web_log( $error_msg );
                return [$result_code,$result_status,$error_msg];
            }
            ...
            if( $num_of_rows != 1 ){
                ...
                $error_msg = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-503",array($strExeTableIdForSelect));
                // ログ出力
                web_log( $error_msg );
                return [$result_code,$result_status,$error_msg];
            }
