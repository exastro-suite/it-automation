...
//
    ...
    //----オーケストレータ別の設定記述
    $strExeTableIdForSelect    = 'C_TERRAFORM_CLI_EXE_INS_MNG';
    $strIfTableIdForSelect     = 'D_TERRAFORM_CLI_IF_INFO';
    $strColIdOfTailLine          = 'TERRAFORM_TAILLOG_LINES';
    $strColIdOfRefreshInt        = 'TERRAFORM_REFRESH_INTERVAL';
    //----オーケストレータ別の設定記述----
    ...
    try{
        ...
        if( !array_key_exists( "execution_no", $_GET ) ){
            // アクセスログ出力(クエリ無し警告)
            web_log( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-401",__FILE__.':'.__LINE__) );
        }
        ...
        if( array_key_exists( "execution_no", $_GET ) === true ){
            ...
            if( $objIntNumVali->isValid($execution_no) === false ){
                // アクセスログ出力(想定外エラー)
                web_log( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-402",__FILE__.':'.__LINE__) );
            }
        }

        //$exec_log  = $objMTS->getSomeMessage("ITATERRAFORMCLI-MNU-150010");
        $error_log = $objMTS->getSomeMessage("ITATERRAFORMCLI-MNU-150020");
        $plan_log = $objMTS->getSomeMessage("ITATERRAFORMCLI-MNU-150030");
        $init_log = $objMTS->getSomeMessage("ITATERRAFORMCLI-MNU-150040");
        $apply_log = $objMTS->getSomeMessage("ITATERRAFORMCLI-MNU-150050");

        $prg_recorder_array = array(1=>array('PRG_RCDR_ID'=>'1'
                                            ,'PRG_RCDR_NAME'=>$init_log
                                            ,'PRG_FILE_NAME'=>'init.log')
        ...
        if( isset($execution_no) ){
            ...
            if($objQuery->getStatus()===false){
                // アクセスログ出力(想定外エラー)
                web_log( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-404",array(__FILE__,__LINE__,"00000100")) );
            ...
            if (!$r){
                // アクセスログ出力(想定外エラー)
                web_log( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-404",array(__FILE__,__LINE__,"00000200")) );
            ...
            if($ret === false) {
                // アクセスログ出力(想定外エラー)
                web_log( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-404",array(__FILE__,__LINE__,"00000400")) );
                unset($objQuery);
            ...
            while ( $row = $objQuery->resultFetch() ){
                ...
                if($ret === false) {
                    // アクセスログ出力(想定外エラー)
                    web_log( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-404",array(__FILE__,__LINE__,"00000900")) );
                    unset($objQuery);
            ...
            if( $num_rows != 1 ){
                // アクセスログ出力(想定外エラー)
                web_log( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-404",array(__FILE__,__LINE__,"00000300")) );
            ...
            ////////////////////////////////////////////////////////////////
            // SQL作成
            $sql = "SELECT {$strColIdOfTailLine} TAILLOG_LINES "
            ...
            if( $objQuery->getStatus()===false ){
                // アクセスログ出力(想定外エラー)
                web_log( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-404",array(__FILE__,__LINE__,"00000600")) );
            ...
            if (!$r){
                // アクセスログ出力(想定外エラー)
                web_log( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-404",array(__FILE__,__LINE__,"00000700")) );
            ...
            if( $num_of_rows != 1 ){
                // アクセスログ出力(想定外エラー)
                web_log( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-404",array(__FILE__,__LINE__,"00000800")) );
    ...
    $aryImportFilePath[] = $objMTS->getTemplateFilePath("ITATERRAFORMCLI","STD","_js");
...
EOD;
    ...
    $strPageExplainBody = $objMTS->getSomeMessage("ITATERRAFORMCLI-MNU-150060");
...
print <<< EOD
    ...
    <h2>
        <table width="100%">
            <tr>
                <!--説明-->
                <td><div onClick=location.href="javascript:show('SetsumeiMidashi','SetsumeiNakami');" class="midashi_class" >{$objMTS->getSomeMessage("ITATERRAFORMCLI-MNU-150070")}</div></td>
                <td>
    ...
    <h2>
        <table width="100%">
            <tr>
                <!--対象作業-->
                <td><div onClick=location.href="javascript:show('ExecutionMidashi','ExecutionNakami');" class="midashi_class" >{$objMTS->getSomeMessage("ITATERRAFORMCLI-MNU-150080")}</div></td>
                <td>
...
print <<< EOD
                ...
                <h2>
                    <table width="100%">
                        <tr>
                            <!--予約取消-->
                            <td><div onClick=location.href="javascript:show('Booking_Cancel_Midashi','Booking_Cancel_Nakami');" class="midashi_class" >{$objMTS->getSomeMessage("ITATERRAFORMCLI-MNU-150090")}</div></td>
                            <td>
                ...
                <div id="Booking_Cancel_Nakami" style="display:block" class="text">
                    <div style="margin-left:10px">
                        <!--予約取消-->
                        <input type="button" id="book_cancel_button" class="updatebutton" value="{$objMTS->getSomeMessage("ITATERRAFORMCLI-MNU-150100")}" onClick=location.href="javascript:book_cancel();" >
                    </div>
                </div>
...
print <<< EOD
                ...
                <h2>
                    <table width="100%">
                        <tr>
                            <!--進行状況-->
                            <td><div onClick=location.href="javascript:show('Monitor_{$loop_counter}_Midashi','Monitor_{$loop_counter}_Nakami');" class="midashi_class" >{$objMTS->getSomeMessage("ITATERRAFORMCLI-MNU-150110")}({$prg_recorder['PRG_RCDR_NAME']})</div></td>
                            <td>
...
print <<< EOD
    ...
    <h2>
        <table width="100%">
            <tr>
                <!--緊急停止-->
                <td><div onClick=location.href="javascript:show('ScramMidashi','ScramNakami');" class="midashi_class" >{$objMTS->getSomeMessage("ITATERRAFORMCLI-MNU-150120")}</div></td>
                <td>
    ...
    <div id="ScramNakami" style="display:block" class="text">
        <div style="margin-left:10px">
            <div id="scram_area">
            <!--緊急停止-->
            <input type="button" id="scrumTryExecute" class="updatebutton scrumSubmitElement tryExecute" value="{$objMTS->getSomeMessage("ITATERRAFORMCLI-MNU-150130")}" onClick=location.href="javascript:scrum_execution();" disabled >
            </div>
        </div>
    </div>
