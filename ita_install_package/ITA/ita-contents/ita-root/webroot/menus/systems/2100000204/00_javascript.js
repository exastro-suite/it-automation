...
// サイト個別、事前処理----
    ...
    show('Editor_Midashi'       ,'Editor_Nakami'        );
    show('Journal1_Midashi'     ,'Journal1_Nakami'      );

// ----サイト個別メニュー、ここから
    show('MenuBrowseMidashi'    ,'MenuBrowseNakami'     );
    document.getElementById('MenuBrowse_area').innerHTML = getSomeMessage("ITAWDCC50101");
// サイト個別メニュー、ここまで----
...
function Mix1_1_disp_menu_list( menu_group_id ){
    // 変数を定義
    var p_menu_group_id = menu_group_id;
    ...
    proxy.dispMenuListHeader( p_menu_group_id );

    //proxy.dispMenuList実行
    proxy.dispMenuList( p_menu_group_id );
}
