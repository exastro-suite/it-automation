...
//
    ...
    try{
        ...
        if( array_key_exists( "symphony_instance_id", $_GET ) === true ){
            ...
            if( $objIntNumVali->isValid($symphony_instance_id) === true ){
                // SQL生成
                $sql = "SELECT  ACCESS_AUTH
                        FROM    C_SYMPHONY_INSTANCE_MNG
                        WHERE   DISUSE_FLAG = '0'
                        AND     SYMPHONY_INSTANCE_NO = :SYMPHONY_INSTANCE_NO_BV ";
                ...
                $objQuery->sqlBind( array( 'SYMPHONY_INSTANCE_NO_BV'=>$symphony_instance_id ) );
