...
//
    ...
    if(array_key_exists("mode",$_GET)===true){
        if($_GET['mode']=="in"){
            ...
            $subConductor = "InputData_Conductor_";
            $subSymphony = "InputData_Symphony_";
            $inOrOut = 'FILE_INPUT';
        }else{
            $filename = "ResultData_Conductor_".$conductorId10.".zip";
            $subConductor = "ResultData_Conductor_";
            $subSymphony = "ResultData_Symphony_";
            $inOrOut = 'FILE_RESULT';
        }
    }
    ...
    if(array_key_exists("mode",$_GET)===true && array_key_exists("conductor_instance_id",$_GET)===true){
        ...
        while(isset($conductorCol[$count]['CONDUCTOR_INSTANCE_NO'])){
            $sql = "SELECT CONDUCTOR_INSTANCE_NO
                    FROM {$conductorInsTable}
                    WHERE CONDUCTOR_CALLER_NO = :CDC_NO ";
             
            //Bindしたいものがあれば配列に入れる。
            $tmpAryBind = array('CDC_NO' => $conductorCol[$count]['CONDUCTOR_INSTANCE_NO']);
        ...
        foreach($conductorCol as $conductorSelect){
            foreach($selectTable as $tableName){
                ...
                $sql = "SELECT EXECUTION_NO, {$inOrOut} ,PATTERN_ID
                        FROM {$tableName}
                        WHERE CONDUCTOR_INSTANCE_NO = :CND_NO ";
                
                //Bindしたいものがあれば配列に入れる。
                $tmpAryBind = array('CND_NO' => $conductorSelect['CONDUCTOR_INSTANCE_NO']);
        ...
        if(!empty($symphonyCol)){
            foreach($symphonyCol as $symphonySelect){
                foreach($selectTable as $tableName){
                    ...
                    //Bindしたいものがあれば配列に入れる。
                    $tmpAryBind = array('SYM_NO' => $symphonySelect['CONDUCTOR_INSTANCE_CALL_NO']);
