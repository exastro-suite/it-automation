...
$tmpFx = function (&$aryVariant=array(),&$arySetting=array()){
    ...
    $table = new TableControlAgent('G_★★★TABLE★★★_CONV_H','ROW_ID', 'No', 'G_★★★TABLE★★★_CONV_H_JNL', $tmpAry);
    $tmpAryColumn = $table->getColumns();
    $tmpAryColumn['ROW_ID']->setSequenceID('F_★★★TABLE★★★_CONV_H_RIC');
    $tmpAryColumn['JOURNAL_SEQ_NO']->setSequenceID('F_★★★TABLE★★★_CONV_H_JSQ');
    unset($tmpAryColumn);

    // ----VIEWをコンテンツソースにする場合、構成する実体テーブルを更新するための設定
    $table->setDBMainTableHiddenID('F_★★★TABLE★★★_CONV_H');
    $table->setDBJournalTableHiddenID('F_★★★TABLE★★★_CONV_H_JNL');
    // 利用時は、更新対象カラムに、「$c->setHiddenMainTableColumn(true);」を付加すること
    // VIEWをコンテンツソースにする場合、構成する実体テーブルを更新するための設定----

    // マルチユニーク制約
    $table->addUniqueColumnSet(array('OPERATION_ID','INPUT_ORDER'));
★★★UNIQUE_CONSTRAINT★★★
...
★★★ITEM★★★
   ...
   //隠し保存テーブルがある場合の、カウント高速化----
    ...
    // 登録/更新/廃止/復活があった場合、パラメータシート縦横変換管理の変換済みフラグをOFFにする
