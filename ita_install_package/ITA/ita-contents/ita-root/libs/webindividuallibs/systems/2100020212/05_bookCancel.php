...
//
    ...
    $strExeJnlTableIdForSelect = 'E_ANSIBLE_PNS_EXE_INS_MNG_JNL';
    ...
    try{
        ...
        if( $retArray[0] === true ){
            ...
            if( $selectRowLength != 1 ){
                $warning_info = $g['objMTS']->getSomeMessage("ITAANSIBLEH-ERR-102070");
        ...
        if( $status_id != 9 ){
            // エラー箇所をメモ
            $warning_info = $g['objMTS']->getSomeMessage("ITAANSIBLEH-ERR-102080",$status_name);
        ...
        // 正常向けの結果メッセージを作成
        $output_str = $g['objMTS']->getSomeMessage("ITAANSIBLEH-STD-102090",$target_execution_no);
    }
    catch (Exception $e){
        //----正常時でも飛ばす版
