...
//
    ...
    // テーブル情報
    $strExeCurTableIdForSelect = 'E_TERRAFORM_CLI_EXE_INS_MNG';
    $strExeJnlTableIdForSelect = 'E_TERRAFORM_CLI_EXE_INS_MNG_JNL';
    ...
    try{
        ...
        if( $retArray[0] === true ){
            ...
            if( $selectRowLength != 1 ){
                $warning_info = $g['objMTS']->getSomeMessage("ITATERRAFORMCLI-ERR-204020");
        ...
        if( $status_id != 9 ){
            // エラー箇所をメモ
            $warning_info = $g['objMTS']->getSomeMessage("ITATERRAFORMCLI-ERR-204030",$status_name);
        ...
        // 正常向けの結果メッセージを作成
        $output_str = $g['objMTS']->getSomeMessage("ITATERRAFORMCLI-STD-201010",$target_execution_no);
    }
    catch (Exception $e){
        //----正常時でも飛ばす版
