...
try{
    ...
    require_once  $g['root_dir_path'] . '/webconfs/systems/2100160103_loadTable.php';

    // メニュー作成用関数群読み込み
    require_once($g['root_dir_path'] . '/libs/webindividuallibs/systems/2100160103/web_functions_for_create_master.php');
...
if (isset($_REQUEST['create']) === false) { // 初期表示時
    try {
          ...
          if(empty($menuNameAry)){
              $resultMsg = $g['objMTS']->getSomeMessage("ITACREPAR-MNU-103504");//作成対象がありません。
          }
    ...
    $resultMsg ="";
    $resultMsg .="<H3>".$g['objMTS']->getSomeMessage("ITACREPAR-MNU-103506")."</H3>";//メニュー作成を受け付けました
    try {
        $resultMsg .="<form method='POST' action='/default/menu/01_browse.php?no=2100160104' id='create_menu_status_form'>";
        $count = 0;
        foreach($_POST as $key => $value){
            if($key !== 'create'){
                $seq = insertCMStatus($key);//登録処理部分
                $resultMsg .=$g['objMTS']->getSomeMessage("ITACREPAR-MNU-103507").":[".$seq."]<BR>";
        ...
        $resultMsg .="<input type='submit' value=".$g['objMTS']->getSomeMessage("ITACREPAR-MNU-103508").">";
        $resultMsg .="</form>";
    } catch (Exception $e) {
