...
function makeMenuCheckbox(){
    ...
    $sql .= ' MENU_NAME ';
    $sql .= 'FROM F_CREATE_MST_MENU_INFO UCMI ';
    $sql .= 'RIGHT JOIN F_CREATE_MST_ITEM_INFO UCII ';
...
function getUserId($username){
    ...
    if ($objQuery === false) {
        ...
        web_log($objQuery->getLastError());
        web_log($g['objMTS']->getSomeMessage('ITABASEH-ERR-900054',
                                             array('A_SEQUENCE', 'F_CREATE_MST_MENU_STATUS_RIC', basename(__FILE__), __LINE__)));
...
function insertCMStatus($create_menu_id){
    ...
    // シーケンスを取得する
    $seq = getSeq("F_CREATE_MST_MENU_STATUS_RIC");

    // 履歴l№を取得する    
    $seqj = getSeq("F_CREATE_MST_MENU_STATUS_JSQ");

    //タイムスタンプ文字列取得
    $tsString = getTimeStampString( sprintf("%6F",microtime(true)));


    $sql  = "INSERT INTO F_CREATE_MST_MENU_STATUS SET ";
    ...
    $sql_j  = "INSERT INTO F_CREATE_MST_MENU_STATUS_JNL SET ";
    ...
    if ($insert_res === false) {
        ...
        web_log($g['objMTS']->getSomeMessage('ITABASEH-ERR-900020',
                                             array('F_CREATE_MST_MENU_STATUS', basename(__FILE__), __LINE__)));
        throw new DBException($g['objMTS']->getSomeMessage('ITABASEH-ERR-900066'));
    }
    if ($insert_res_j === false) {
        ...
        web_log($g['objMTS']->getSomeMessage('ITABASEH-ERR-900020',
                                             array('F_CREATE_MST_MENU_STATUS_JNL', basename(__FILE__), __LINE__)));
        throw new DBException($g['objMTS']->getSomeMessage('ITABASEH-ERR-900066'));
    }

    //シーケンス更新
    updateSeq("F_CREATE_MST_MENU_STATUS_RIC");
    updateSeq("F_CREATE_MST_MENU_STATUS_JSQ");
