...
//通知ステータスリスト用の取得
function printNoticeStatusListInfoRegConductor(){
    global $g;
    ...
    $aryRowOfNoticeStatussTable=array();
    ...
    try{
        ...
        $aryRetBody = $objOLA->getInfoOfNoticeStatusList();
        if( $aryRetBody[1]!==null ){
            // 例外処理へ
            $strErrStepIdInFx="00001000";
            $intErrorType = $aryRetBody[1];
        ...
        foreach ($aryRetBody[4] as $key => $value) {
            $aryRowOfNoticeStatussTable[]=array(
                'STATUS_ID' =>  htmlspecialchars($value['SYM_EXE_STATUS_ID'])
                ,'STATUS_NAME'   =>  htmlspecialchars($value['SYM_EXE_STATUS_NAME'])
                #,'DEFAULT' => ""
            );
            
        }
        $strStreamOfOperation = json_encode($aryRowOfNoticeStatussTable,JSON_UNESCAPED_UNICODE);
    }
    catch (Exception $e){
        // エラーフラグをON
        if( $intErrorType===null ) $intErrorType = 500;
