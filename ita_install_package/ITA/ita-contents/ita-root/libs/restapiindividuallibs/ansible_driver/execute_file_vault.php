...
//
    ...
    $tmp_file       = sprintf("/tmp/%s_%s_%s.rsa",$in_OrchestratorSub_Id,$in_ExeNo,getmypid());


    $EncodeValue = "";
    $this->arySuccessInfo['status'] = "FAILED";
    // 暗号化するデータを一時ファイルに保存
    $ret = file_put_contents($tmp_file,$in_TargetValue);
    if($ret === false) {
        unlink($tmp_file);
        $EncodeValue = 'Failed to create temporary file used by ansible vault.';
        $this->arySuccessInfo['status'] = "FAILED";
    } else {
        $vaultData = "";
        // ansible-vaultで暗号化
        // $tmp_fileは$obj->FileVault内で削除
        $ret = $obj->FileVault($in_ExecUser,$PasswordFile,$tmp_file,$vaultData,$EncodeValue,'');
        if ($ret !== true) {
            $EncodeValue = 'Failed to ansible vault for ssh key file.';
            $this->arySuccessInfo['status'] = "FAILED";
        } else {
            $EncodeValue = $vaultData;
