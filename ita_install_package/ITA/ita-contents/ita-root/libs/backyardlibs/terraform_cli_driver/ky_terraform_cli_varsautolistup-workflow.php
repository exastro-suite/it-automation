...
//
    ...
    $db_access_user_id   = -101903; //Terraform-CLI変数更新プロシージャ
    ...
    $lv_a_proc_loaded_list_varsetup_pkey = 2100080003;
    $lv_a_proc_loaded_list_valsetup_pkey = 2100080004;
    ...
    try{
        ...
        if($log_level === "DEBUG") {
            ...
            $traceMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-202100");
        ...
        if($ret === false) {
            ...
            $errorMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-206090");
        ...
        if(count($lv_UpdateRecodeInfo) == 0) {
            ...
            if($log_level === "DEBUG") {
                ...
                $traceMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-202110");
                LocalLogPrint(basename(__FILE__),__LINE__,$traceMsg);
            }
            exit(0);
        }
        ...
        if ( $log_level === 'DEBUG' ){
            // [処理]トランザクション開始
            $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-202060");
            require ($root_dir_path . $log_output_php );
        }
        ...
        $strTableCurTerraformMatter    = "B_TERRAFORM_CLI_MODULE";
        ...
        //----------------------------------------------
        // SQL生成 Module素材テーブル(B_TERRAFORM_CLI_MODULE)
        //----------------------------------------------
        $sqlUtnBody = "SELECT "
                     ." {$strColumnIdOfMatterId} MATTER_ID,"
                     ." {$strColumnIdOfMatterFile} MATTER_FILE, "
                     ." ACCESS_AUTH "
                     ."FROM {$strTableCurTerraformMatter} "     // リソーステーブル(B_TERRAFORM_CLI_MODULE)
                     ."WHERE DISUSE_FLAG = '0' ";
        ...
        if( $objQueryUtn->getStatus()===false ){
            ...
            // 異常フラグON  例外処理へ
            $error_flag = 1;
            throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-101010",array(__FILE__,__LINE__,"00000300")) );
        }
        if( $objQueryUtn->sqlBind($arrayUtnBind) != "" ){
            ...
            $error_flag = 1;
            // 例外処理へ
            throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-101010",array(__FILE__,__LINE__,"00000400")) );
        }
        //----------------------------------------------
        // SQL実行 Module素材テーブル(B_TERRAFORM_CLI_MODULE)
        //----------------------------------------------
        ...
        if (!$r){
            ...
            // 異常フラグON  例外処理へ
            $error_flag = 1;
            throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-101010",array(__FILE__,__LINE__,"00000500")) );
        }
        //----------------------------------------------
        ...
        foreach($aryMatterFilePerMatterId as $intMatterId=>$strMatterFile){
            ...
            if(strlen($strMatterFile)===0){
                ...
                $msgstr = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-206050",array($intMatterId));
        ...
        //B_TERRAFORM_CLI_MODULE_VARS_LINKテーブルのレコードを格納する配列を宣言
        ...
        //----------------------------------------------
        // SQL作成  Module変数紐付けテーブル  B_TERRAFORM_CLI_MODULE_VARS_LINK
        //----------------------------------------------
        ...
        if( $objQueryUtn->getStatus()===false ){
            ...
            // 異常フラグON  例外処理へ
            $error_flag = 1;
            throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-101010",array(__FILE__,__LINE__,"00000600")) );
        }
        if( $objQueryUtn->sqlBind($arrayUtnBind) != "" ){
            $FREE_LOG = sprintf("FILE:%s LINE:%s %s",basename(__FILE__),__LINE__,
                                $objQueryUtn->getLastError());
            ...
            // 異常フラグON  例外処理へ
            $error_flag = 1;
            throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-101010",array(__FILE__,__LINE__,"00000700")) );
        }
        //----------------------------------------------
        // SQL実行
        //----------------------------------------------
        ...
        foreach($aryModuleVarsData as $data){
            ...
            if (is_array($type_array)) {
                        ...
                        if ($tmp_member_data["max_col_seq"] > 0) {
                            ...
                            if (!$registedMaxMemberColData["isRegist"]) {
                                ...
                                if (!$res) {
                                    ...
                                    $errorMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-206120", array(__FILE__, __LINE__));
                                    throw new Exception($errorMsg);
                                }
                            }
                            // 登録がある場合
                                ...
                                if ($registedMaxMemberColData["maxColSeq"] - $tmp_member_data["max_col_seq"] != 0 && $registedMaxMemberColData["isSystem"] == true) {
                                    ...
                                    if (!$res) {
                                        ...
                                        $errorMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-206130");
                                        throw new Exception($errorMsg);
                                    }
                                }
                                // 最終更新者がシステム且つ廃止状態の場合
                                elseif ($registedMaxMemberColData["DISUSE_FLAG"] == 1 && $registedMaxMemberColData["isSystem"] == true)
                                {
                                    ...
                                    if (!$res) {
                                        // 「変数ネスト管理の最大繰り返し数の更新に失敗しました。」
                                        $errorMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-206130", array(__FILE__, __LINE__, $registedMaxMemberColData["MAX_COL_SEQ_ID"]));
                                        throw new Exception($errorMsg);
                                    }
                                }
                                //アクセス許可情報が変更されている場合
                ...
                if (!empty($member_array["regist"])) {
                    foreach ($member_array["regist"] as $member_data) {
                        // 既にB_TERRAFORM_CLI_VAR_MEMBERテーブルにあるレコードを取得
                        ...
                        */
                        ...
                        if (!$res) {
                            $error_flag = 1;
                            // 「メンバー変数の登録に失敗しました。」
                            $errorMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-206140", array(__FILE__, __LINE__, $member_data["CHILD_MEMBER_VARS_ID"]));
                            throw new Exception($errorMsg);
                        }
                    // typeが変数ネスト管理の対象である
                ...
                if (!empty($member_array["update"])) {
                    foreach ($member_array["update"] as $member_data) {
                        ...
                        if (!$res) {
                            $error_flag = 1;
                            // 「メンバー変数の更新に失敗しました。」
                            $errorMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-206150", array(__FILE__, __LINE__));
                            throw new Exception($errorMsg);
                        }
                        // 変数ネスト管理とtfファイルから取得した最大繰り返し数に差分があり、最終更新者がシステムの場合は変数ネスト管理の値を更新
                        if ($member_data["MAX_COL_SEQ"] > 0) {
                            ...
                            if ($registedMaxMemberColData["isRegist"] == true && $registedMaxMemberColData["DISUSE_FLAG"] == 0  && $registedMaxColseq != $tfMaxColseq && $registedMaxMemberColData["isSystem"] == true) {
                                ...
                                if (!$res) {
                                    // 「変数ネスト管理の最大繰り返し数の更新に失敗しました。」
                                    $errorMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-206130", array(__FILE__, __LINE__, $member_data["MAX_COL_SEQ"]));
                                    throw new Exception($errorMsg);
                                }
                            }
                        }
                    ...
                    foreach ($member_array["restore"] as $member_data) {
                        ...
                        if ($member_data["MAX_COL_SEQ"] > 0) {
                            ...
                            if ($registedMaxMemberColData["isRegist"]) {
                                ...
                                if(!$res) {
                                    // 「変数ネスト管理の最大繰り返し数の更新に失敗しました。」
                                    $errorMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-206140", array(__FILE__, __LINE__, $registedMaxMemberColData["MAX_COL_SEQ_ID"]));
                                    throw new Exception($errorMsg);
                                }
                            }
                            else {
                    ...
                    foreach ($member_array["skip"] as $member_data) {
                        ...
                        if ($member_data["MAX_COL_SEQ"] > 0) {
                            ...
                            if ($registedMaxMemberColData["isRegist"] == true && $registedMaxColseq != $tfMaxColseq && $registedMaxMemberColData["isSystem"] == true) {
                                ...
                                "ITATERRAFORMCLI-ERR-206130", array(__FILE__, __LINE__, $registedMaxMemberColData["MAX_COL_SEQ_ID"]));
                                    throw new Exception($errorMsg);
                                }
                            }
                            elseif($registedMaxMemberColData["isRegist"] == true && $registedMaxMemberColData["DISUSE_FLAG"] == 1 && $registedMaxMemberColData["isSystem"] == true) {
            ...
            if ( $log_level === 'DEBUG' ){
                ...
                ob_clean();
                LocalLogPrint(basename(__FILE__),__LINE__, $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-202040") . "\n" . $msgstr);
            }
            ...
            if( $objQueryUtn->sqlBind($arrayUtnBind) != "" ){
                ...
                // 異常フラグON  例外処理へ
                $error_flag = 1;
                throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-101010",array(__FILE__,__LINE__,"00001600")) );
            }
            if( $objQueryJnl->sqlBind($arrayJnlBind) != "" ){
                ...
                // 異常フラグON  例外処理へ
                $error_flag = 1;
                throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-101010",array(__FILE__,__LINE__,"00001700")) );
            }
            //----------------------------------------------
            // SQL実行  objQueryUtn
            //----------------------------------------------
            ...
            if($rUtn!=true){
                ...
                // 異常フラグON  例外処理へ
                $error_flag = 1;
                throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-101010",array(__FILE__,__LINE__,"00001800")) );
            }
            ...
            if($rJnl!=true){
                ...
                // 異常フラグON  例外処理へ
                $error_flag = 1;
                throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-101010",array(__FILE__,__LINE__,"00001900")) );
            }
        ...
        foreach($aryRowFromTerraformVarsTable as $row){
            ...
            if($boolExistFlat == false){
                ...
                if( $aryRowOfTableUpdate["DISUSE_FLAG"] == "0" ){
                    ...
                    // 異常フラグON  例外処理へ
                    $error_flag = 1;
                    throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-101010",array(__FILE__,__LINE__,"00002000")) );
                    //想定外エラー----
                }
                ...
                if( $retArray[1] != 0 ){
                    // 異常フラグON  例外処理へ
                    $error_flag = 1;
                    throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-101010",array(__FILE__,__LINE__,"00002100")) );
                }
                // テーブル シーケンスNoを採番
                $retArray = getSequenceValueFromTable($strSeqOfJnlTableTerraformVars, 'A_SEQUENCE', FALSE );
                if( $retArray[1] != 0 ){
                    // 異常フラグON  例外処理へ
                    $error_flag = 1;
                    throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-101010",array(__FILE__,__LINE__,"00002200")) );
                }
                ...
                if ( $log_level === 'DEBUG' ){
                    ...
                    $msgstr = ob_get_contents();
                    ob_clean();
                    LocalLogPrint(basename(__FILE__),__LINE__, $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-202050") . "\n" . $msgstr);
                }
                ...
                if( $objQueryUtn->getStatus()===false ){
                    ...
                    // 異常フラグON  例外処理へ
                    $error_flag = 1;
                    throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-101010",array(__FILE__,__LINE__,"00002300")) );
                }
                if( $objQueryJnl->getStatus()===false ){
                    $FREE_LOG = sprintf("FILE:%s LINE:%s %s",basename(__FILE__),__LINE__,
                                        $objQueryJnl->getLastError());
                    ...
                    // 異常フラグON  例外処理へ
                    $error_flag = 1;
                    throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-101010",array(__FILE__,__LINE__,"00002400")) );
                }
                if( $objQueryUtn->sqlBind($arrayUtnBind) != "" ){
                    $FREE_LOG = sprintf("FILE:%s LINE:%s %s",basename(__FILE__),__LINE__,
                                        $objQueryUtn->getLastError());
                    ...
                    // 異常フラグON  例外処理へ
                    $error_flag = 1;
                    throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-101010",array(__FILE__,__LINE__,"00002500")) );
                }
                ...
                if( $objQueryJnl->sqlBind($arrayJnlBind) != "" ){
                    ...
                    // 異常フラグON  例外処理へ
                    $error_flag = 1;
                    throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-101010",array(__FILE__,__LINE__,"00002600")) );
                }
                $rUtn = $objQueryUtn->sqlExecute();
                if($rUtn!=true){
                    $FREE_LOG = sprintf("FILE:%s LINE:%s %s",basename(__FILE__),__LINE__,
                    ...
                    // 異常フラグON  例外処理へ
                    $error_flag = 1;
                    throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-101010",array(__FILE__,__LINE__,"00002700")) );
                }
                $rJnl = $objQueryJnl->sqlExecute();
                if($rJnl!=true){
                    $FREE_LOG = sprintf("FILE:%s LINE:%s %s",basename(__FILE__),__LINE__,
                    ...
                    // 異常フラグON  例外処理へ
                    $error_flag = 1;
                    throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-101010",array(__FILE__,__LINE__,"00002800")) );
                }
                //廃止する----
                // DBアクセス事後処理
                unset($objQueryUtn);
        ...
        if (!$r){
            ...
            $error_flag = 1;
            // 例外処理へ
            throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-101010",array(__FILE__,__LINE__,"00002900")) );
        }
        // トレースメッセージ
        if ( $log_level === 'DEBUG' ){
            // [処理]コミット;
            $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-202080");
            require ($root_dir_path . $log_output_php );
        }
        ...
        if ( $log_level === 'DEBUG' ){
            // [処理]トランザクション終了
            $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-202070");
            require ($root_dir_path . $log_output_php );
        }
        ...
        if($log_level === "DEBUG") {
            ...
            $traceMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-202120");
            LocalLogPrint(basename(__FILE__),__LINE__,$traceMsg);
        }
        ...
        if($ret === false) {
            ...
            $errorMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-206100");
            throw new Exception($errorMsg);
        }
        ...
        if($db_update_flg === true) {
            if($log_level === "DEBUG") {
                ...
                $traceMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-202130");
                LocalLogPrint(basename(__FILE__),__LINE__,$traceMsg);
            }
            ...
            if($ret === false) {
                ...
                $errorMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-206110");
                throw new Exception($errorMsg);
            }
        }
        ...
        foreach ($disusedModuleIDArray as $disabledModuleID) {
            ...
            foreach ($disabledMemberVarsArray as $disabledMemberVars) {
                ...
                if (!$res) {
                    $error_flag = 1;
                    // 「メンバー変数の廃止に失敗しました。」
                    $errorMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-206170", array(__FILE__, __LINE__, $disabledMemberVars["CHILD_MEMBER_VARS_ID"]));
                    throw new Exception($errorMsg);
                }
            }
            ...
            if (!empty($disabledMaxColSeqVarsArray)) {
                foreach ($disabledMaxColSeqVarsArray as $disabledMaxColSeqVars) {
                    ...
                    if (!$res) {
                        $error_flag = 1;
                        // 「最大繰り返し数の廃止に失敗しました。」
                        $errorMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-206140", array(__FILE__, __LINE__, $disabledMaxColSeqVars["MAX_COL_SEQ_ID"]));
                        throw new Exception($errorMsg);
                    }
                }
            }
        ...
        foreach ($memberVarsArray as $memberVars) {
            ...
            if ($deleteFlag) {
                ...
                if (!$res) {
                    $error_flag = 1;
                    // 「メンバー変数の廃止に失敗しました。」
                    $errorMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-206160", array(__FILE__, __LINE__, $memberVars["CHILD_MEMBER_VARS_ID"]));
                    throw new Exception($errorMsg);
                }
        ...
        // 例外発生
        $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-206060");
        require ($root_dir_path . $log_output_php );
        ...
        if( $objDBCA->getTransactionMode() ){
            ...
            if( $objDBCA->transactionRollBack()=== true ){
                // [処理]ロールバック
                $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-202090");
            }
            else{
                // ロールバックに失敗しました
                $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-206020");
            }
            require ($root_dir_path . $log_output_php );
            ...
            if( $objDBCA->transactionExit()=== true ){
                ...
                $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-206010");
            }
            require ($root_dir_path . $log_output_php );
        }
    ...
    elseif( $warning_flag != 0 ){
        ...
        if ( $log_level === 'DEBUG' ){
            // プロシージャ終了(警告)
            $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-206040");
            require ($root_dir_path . $log_output_php );
        }
        exit(0);
    }
    ...
    function getHostVars($in_pkey,
        ...
        if( file_exists($file_name) === false ){
            ...
            $msgstr = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-206070",array($in_pkey,basename($in_filename)));
            LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
    ...
    function recordSelect($sqlUtnBody, $arrayUtnBind) {
        ...
        $objQueryUtn = $objDBCA->sqlPrepare($sqlUtnBody);
        if($objQueryUtn->getStatus()===false) {
            $msgstr = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-206080",array(basename(__FILE__),__LINE__));
            LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
    ...
    function registMemberVars($record_data)
    {
        ...
        //B_TERRAFORM_CLI_VAR_MEMBERテーブルのレコードを格納する配列を宣言
        ...
        //----------------------------------------------
        // SQL作成  メンバー変数紐付け  B_TERRAFORM_CLI_VAR_MEMBER
        //----------------------------------------------
        $retArray = makeSQLForUtnTableUpdate($db_model_ch,
                                            "INSERT",
                                            "CHILD_MEMBER_VARS_ID",
    ...
    function getMemberVarsByMemberData($member_data)
    {
        ...
        if (isset($member_data["PARENT_MEMBER_VARS_ID"])) {
            $sqlUtnBody = "SELECT * "
            . "FROM {$strCurTableTerraformMemberVarsTable} "     // メンバー変数テーブル(B_TERRAFORM_CLI_VAR_MEMBER)
            . "WHERE "
        ...
        //----------------------------------------------
        // SQL実行 メンバー変数管理(B_TERRAFORM_CLI_VAR_MEMBER)
        //----------------------------------------------
        $r = $objQueryUtn->sqlExecute();
        if (!$r) {
            LocalLogPrint(basename(__FILE__),__LINE__,$objQueryUtn->getLastError());
    ...
    function getMaxColSeqVarsByModuleID($moduleID)
    {
        ...
        $sqlUtnBody = "SELECT * "
            . "FROM {$strCurTableTerraformMaxMemberColTable} "     // メンバー変数テーブル(B_TERRAFORM_CLI_LRL_MAX_MEMBER_COL)
            . "WHERE DISUSE_FLAG = '0' "
    ...
    function getTypeID($strType)
    {
        ...
        $sqlUtnBody = "SELECT "
        . " TYPE_ID "
        . "FROM {$vg_terraform_types_master} "     // タイプマスタ(B_TERRAFORM_CLI_TYPES_MASTER)
        . "WHERE DISUSE_FLAG = '0' "
    ...
    function getTypeInfo($typeID)
    {
        ...
        $sqlUtnBody = "SELECT "
            . " * "
            . "FROM {$vg_terraform_types_master} "     // リソーステーブル(B_TERRAFORM_CLI_MODULE)
            . "WHERE DISUSE_FLAG = '0' "
            . "AND TYPE_ID = :TYPE_ID ";
    ...
    //*******************************************************************************************
    //----Module変数のTypeのIDを取得する(B_TERRAFORM_CLI_MODULE_VARS_LINK)
    //*******************************************************************************************
    function getModuleVarsLinkTypeInfo($moduleLinkVarsID)
    {
        ...
        $sqlUtnBody = "SELECT "
            . " * "
            . "FROM {$vg_terraform_module_vars_link_table_name} "     // リソーステーブル(B_TERRAFORM_CLI_MODULE_VARS_LINK)
            . "WHERE DISUSE_FLAG = '0' "
    ...
    function registMaxMemberCol($vars_id, $member_vars_id, $max_col_seq, $access_auth) {
        $res = false;
        global $strCurTableTerraformMaxMemberColTable; // B_TERRAFORM_CLI_LRL_MAX_MEMBER_COL
        ...
        //----------------------------------------------
        // SQL作成  変数ネスト管理  B_TERRAFORM_CLI_LRL_MAX_MEMBER_COL
        //----------------------------------------------
        $retArray = makeSQLForUtnTableUpdate($db_model_ch,
                                            "INSERT",
                                            "MAX_COL_SEQ_ID",
    ...
    function getRegistMaxModuleColData($moduleLinkVarsID) {
        ...
        $sqlUtnBody = "SELECT "
            . " * "
            . "FROM {$vg_terraform_max_member_col_table_name} "     // 変数ネスト管理(B_TERRAFORM_CLI_LRL_MAX_MEMBER_COL)
        ...
        //----------------------------------------------
        // SQL実行  変数ネスト管理(B_TERRAFORM_CLI_LRL_MAX_MEMBER_COL)
        //----------------------------------------------
        $r = $objQueryUtn->sqlExecute();
        if (!$r) {
            LocalLogPrint(basename(__FILE__),__LINE__,$objQueryUtn->getLastError());
    ...
    function updateMaxColSeq($maxMemberColID, $maxColSeq, $access_auth) {
        ...
        //----------------------------------------------
        // SQL実行 変数ネスト管理テーブル(B_TERRAFORM_CLI_LRL_MAX_MEMBER_COL)
        //----------------------------------------------
        $r = $objQueryUtn->sqlExecute();
        if (!$r) {
            LocalLogPrint(basename(__FILE__),__LINE__,$objQueryUtn->getLastError());
        ...
        //B_TERRAFORM_CLI_LRL_MAX_MEMBER_COLテーブルのレコードを格納する配列を宣言
