...
//
    ...
    $db_access_user_id   = -101904; //Terraform代入値自動登録設定プロシージャ
    ...
    ////////////////////////////////////////////////////////////////
    //----変数一覧(B_TERRAFORM_CLI_MODULE_VARS_LINK)
    ////////////////////////////////////////////////////////////////
    ...
    ////////////////////////////////////////////////////////////////
    //----作業パターン詳細(B_TERRAFORM_CLI_PATTERN_LINK)
    ////////////////////////////////////////////////////////////////
    ...
    try{
        ...
        if($log_level === "DEBUG") {
            ...
            $traceMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-203010");
        ...
        if($ret === false) {
            ...
            $errorMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207020");
        ...
        if(count($lv_UpdateRecodeInfo) == 0) {
            ...
            if($log_level === "DEBUG") {
                ...
                $traceMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-203020");
                LocalLogPrint(basename(__FILE__),__LINE__,$traceMsg);
            }
            exit(0);
        }
        ...
        if($ret === false) {
            $errorMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207340"); //マスタデータのアクセス許可ロールの取得に失敗しました。
            throw new Exception($errorMsg);
        }
        ...
        if ( $log_level === 'DEBUG' ){
            ...
            $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-203050");
            LocalLogPrint(basename(__FILE__),__LINE__,$FREE_LOG);
        }
        // テーブル名配列
        ...
        if($ret === false){
            $error_flag = 1;
            $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207050");
        ...
        if ( $log_level === 'DEBUG' ){
            ...
            $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-203060");
            LocalLogPrint(basename(__FILE__),__LINE__,$FREE_LOG);
        }
        ...
        if($log_level === "DEBUG") {
            //[処理]トランザクション開始
            $traceMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-203170");
            LocalLogPrint(basename(__FILE__),__LINE__,$traceMsg);
        }
        ...
        if($objDBCA->transactionStart()===false) {
            ...
            $error_flag = 1;
            //トランザクションスタートが失敗しました。
            throw new Exception($objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207280"));
        }
        ...
        if($ret === false) {
            ...
            $error_flag = 1;
            //シーケンスロックに失敗しました。
            $errorMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207290");
            throw new Exception($errorMsg);
        }
        ...
        if($ret === false) {
            ...
            $error_flag = 1;
            //代入値管理の読込に失敗しました。
            $errorMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207060");
            throw new Exception($errorMsg);
        }
        ...
        if ( $log_level === 'DEBUG' ){
            ...
            $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-203070");
            LocalLogPrint(basename(__FILE__),__LINE__,$FREE_LOG);
        }
        ...
        foreach($lva_vars_ass_list as $vars_ass_list){
            ...
            if($ResultAccessAuthStr === false) {
                if($log_level === "DEBUG") {
                    ...
                    $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207350",
            ...
            if($ret === false){
                ...
                $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207070");
                throw new Exception( $FREE_LOG );
            }
        }
        ...
        if ( $log_level === 'DEBUG' ){
            //[処理]代入値管理から不要なデータを削除
            $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-203080");
            LocalLogPrint(basename(__FILE__),__LINE__,$FREE_LOG);
        }
        ...
        if($ret === false){
            // 異常フラグON
            $error_flag = 1;
            $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207080");
            throw new Exception( $FREE_LOG );
        }
        ...
        if($log_level === "DEBUG") {
            //[処理]コミット
            $traceMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-203190");
            LocalLogPrint(basename(__FILE__),__LINE__,$traceMsg);
        }
        ...
        if(!$r) {
            ...
            $errorMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207300");
            throw new Exception($errorMsg);
        }
        ...
        if($log_level === "DEBUG") {
            //[処理]トランザクション終了
            $traceMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-203180");
            LocalLogPrint(basename(__FILE__),__LINE__,$traceMsg);
        }
        ...
        if (!$r){
            ...
            // 例外処理へ
            //トランザクションのコミットに失敗しました。
            throw new Exception( $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207300"));
        }
        ...
        if ( $log_level === 'DEBUG' ){
            //[処理]トランザクション終了
            $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-203180");
            LocalLogPrint(basename(__FILE__),__LINE__,$FREE_LOG);
        }
        ...
        if($log_level === "DEBUG") {
            ...
            $traceMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-203030");
            LocalLogPrint(basename(__FILE__),__LINE__,$traceMsg);
        }
        ...
        if($ret === false) {
            ...
            $errorMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207030");
            throw new Exception($errorMsg);
        }
        ...
        if($db_update_flg === true) {
            if($log_level === "DEBUG") {
                ...
                $traceMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-203040");
                LocalLogPrint(basename(__FILE__),__LINE__,$traceMsg);
            }
            ...
            if($ret === false) {
                ...
                $errorMsg = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207040");
                throw new Exception($errorMsg);
            }
        }
    }
        ...
        //例外が発生しました。
        $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207310");
        LocalLogPrint(basename(__FILE__),__LINE__,$FREE_LOG);
        ...
        if( $objDBCA->getTransactionMode() ){
            ...
            if( $objDBCA->transactionRollBack()=== true ){
                //[処理]ロールバック
                $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-203200");
            }
            else{
                //ロールバックに失敗しました。
                $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207320");
            }
            LocalLogPrint(basename(__FILE__),__LINE__,$FREE_LOG);
            ...
            if( $objDBCA->transactionExit()=== true ){
                ...
                $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207330");
            }
            LocalLogPrint(basename(__FILE__),__LINE__,$FREE_LOG);
        }
    }
                          ...
                          &$ina_table_nameTOPkeyname_list)
    {
        ...
        if($objQuery->getStatus()===false){
            $msgstr = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207010",array(basename(__FILE__),__LINE__));
        ...
        while ( $row = $objQuery->resultFetch() ){
            ...
            if($row['TBL_DISUSE_FLAG'] != '0'){
                if ( $log_level === 'DEBUG' ){
                    ...
                    $msgstr = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207090",array($row['COLUMN_ID']));
                    LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
                }
                // 次のカラムへ
                continue;
            ...
            if($row['COL_DISUSE_FLAG'] != '0'){
                if ( $log_level === 'DEBUG' ){
                    ...
                    $msgstr = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207100",array($row['COLUMN_ID']));
                    LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
                }
                // 次のカラムへ
                continue;
            ...
            if($row['PATTERN_CNT'] == 0){
                if ( $log_level === 'DEBUG' ){
                    ...
                    $msgstr = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207110",array($row['COLUMN_ID']));
                    LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
                }
                // 次のカラムへ
                continue;
            ...
            if(@strlen($row['TABLE_NAME']) == 0){
                if ( $log_level === 'DEBUG' ){
                    ...
                    $msgstr = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207120",array($row['COLUMN_ID']));
                    LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
                }
                // 次のカラムへ
                continue;
            ...
            if(@strlen($row['PKEY_NAME']) == 0){
                if ( $log_level === 'DEBUG' ){
                    ...
                    $msgstr = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207130",array($row['COLUMN_ID']));
                    LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
                }
                // 次のカラムへ
                continue;
            ...
            if(@strlen($row['COL_NAME']) == 0){
                if ( $log_level === 'DEBUG' ){
                    ...
                    $msgstr = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207140",array($row['COLUMN_ID']));
                    LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
                }
                // 次のカラムへ
                continue;
            ...
            if(@strlen($row['COL_TITLE']) == 0){
                if ( $log_level === 'DEBUG' ){
                    ...
                    $msgstr = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207150",array($row['COLUMN_ID']));
                    LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
                }
                // 次のカラムへ
                continue;
            ...
            default:
                if ( $log_level === 'DEBUG' ){
                    //
                    $msgstr = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207160",array($row['COLUMN_ID'],$row['COL_TYPE']));
                    LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
                }
                // 次のカラムへ
                continue 2;
    ...
    function ColVarInfoAnalysis($in_col_type,           //カラムタイプ Value/Key
        ...
        if(@strlen($row[$in_vars_link_id]) == 0){
            if ( $log_level === 'DEBUG' ){
                ...
                $msgstr = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207170",array($row['COLUMN_ID'],$in_col_type));
                LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
            }
            // エラーリターン
            return false;
        ...
        if($row[$in_ptn_vars_link_cnt] == 0){
            if ( $log_level === 'DEBUG' ){
                ...
                $msgstr = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207180",array($row['COLUMN_ID'],$in_col_type));
                LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
            }
            // エラーリターン
            return false;
        ...
        if(@strlen($row[$in_vars_name]) == 0){
            if ( $log_level === 'DEBUG' ){
                ...
                $msgstr = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207190",array($row['COLUMN_ID'],$in_col_type));
                LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
            }
            // エラーリターン
            return false;
        ...
        foreach($ina_table_colnameTOid_list as $table_name=>$col_list){
            ...
            if($make_sql == ""){
                ...
                if ( $log_level === 'DEBUG' ){
                    ...
                    $msgstr = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207200",array($ina_table_nameTOid_list[$table_name]));
                    LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
                }
                //次のテーブルへ
                continue;
    ...
    function GetMenuData($ina_table_nameTOsql_list,
        ...
        foreach($ina_table_nameTOsql_list as $table_name=>$sql){
            if ( $log_level === 'DEBUG' ){
                ...
                $msgstr = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-203090",array($ina_table_nameTOid_list[$table_name]));
                LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
            }
            ...
            if($ret === false){
                ...
                if ( $log_level === 'DEBUG' ){
                    ...
                    $msgstr = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207210",array($ina_table_nameTOid_list[$table_name]));
                    LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
                }
                ...
                if( ! isset($total_row)){
                    ...
                    if ( $log_level === 'DEBUG' ){
                        ...
                        $msgstr = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207220",array($ina_table_nameTOid_list[$table_name]));
                        LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
                    }
                    //次のテーブルへ
                    continue;
                ...
                foreach($total_row as $row){
                    ...
                    if(@strlen($row['OPERATION_ID']) == 0){
                        ...
                        if ( $log_level === 'DEBUG' ){
                            ...
                            $msgstr = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207230",array($ina_table_nameTOid_list[$table_name],$row[DF_ITA_LOCAL_PKEY]));
                            LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
                        }
    ...
    function chkVarsAssData($in_table_name,
        ...
        if( isset($ina_vars_ass_chk_list[$in_operation_id]
            ...
            $msgstr = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207240",array( $dup_info['COLUMN_ID'],
                                                                             $in_column_id,
                                                                             $in_column_id,
                                                                             $in_operation_id,
                                                                             $in_key_value_vars_id,
    ...
    function addStg1VarsAssDB($ina_varsass_list,&$in_VarsAssignRecodes) {
        ...
        $strCurTable      = $strCurTableVarsAss; //B_TERRAFORM_CLI_VARS_ASSIGN
        ...
        if( ! isset($in_VarsAssignRecodes[$key])) {
            ...
            if($tgt_row["VARS_ENTRY"]  == $ina_varsass_list['VARS_ENTRY']){
                ...
                if ( $log_level === 'DEBUG' ){
                    ...
                    $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-203130",
            ...
            if($tgt_row["LAST_UPDATE_USER"] != $db_access_user_id){
                ...
                if ( $log_level === 'DEBUG' ){
                    ...
                    $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-203160",
                                                        array($ina_varsass_list['OPERATION_NO_UAPK'],
                                                              $ina_varsass_list['PATTERN_ID'],
                                                              $ina_varsass_list['MODULE_VARS_LINK_ID'],
                                                              $ina_varsass_list['MEMBER_VARS'],
            ...
            if ( $log_level === 'DEBUG' ){
                ...
                $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-203120",
                                            array($ina_varsass_list['OPERATION_NO_UAPK'],
                                                  $ina_varsass_list['PATTERN_ID'],
                                                  $ina_varsass_list['MODULE_VARS_LINK_ID'],
                                                  $ina_varsass_list['MEMBER_VARS'],
        ...
        if($action == "UPDATE"){
            ...
            if( $retArray[1] != 0 ){
                $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207010",array(basename(__FILE__),__LINE__));
                LocalLogPrint(basename(__FILE__),__LINE__,$FREE_LOG);
        ...
        foreach($in_VarsAssignRecodes as $key=>$tgt_row) {
            ...
            if($tgt_row["LAST_UPDATE_USER"] != $db_access_user_id){
                ...
                if ( $log_level === 'DEBUG' ){
                    ...
                    $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-203150",
            ...
            if ( $log_level === 'DEBUG' ){
                ...
                $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-203140",
                                                       array($tgt_row['ASSIGN_ID']));
                LocalLogPrint(basename(__FILE__),__LINE__,$FREE_LOG);
            }
        ...
        if(strlen($in_col_val)==0){
            ...
            if(getNullDataHandlingID($in_null_data_handling_flg) != '1')
            {
                 ...
                 if ( $log_level === 'DEBUG' ){
                    ...
                    $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207250",
        ...
        if(! isset($in_VarsAssignRecodes[$key]))
        {
             ...
             if ( $log_level === 'DEBUG' ){
                //[処理]代入値管理 追加 (OPERATION_ID:{} PATTERN_ID:{} MODULE_VARS_LINK_ID:{})
                 $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-203100",
                                                     array($ina_varsass_list['OPERATION_NO_UAPK'],
                                                           $ina_varsass_list['PATTERN_ID'],
                                                           $ina_varsass_list['MODULE_VARS_LINK_ID'],
                                                           $ina_varsass_list['MEMBER_VARS'],
            ...
            if ( $log_level === 'DEBUG' ){
                ...
                $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-STD-203110",
                                                array($ina_varsass_list['OPERATION_NO_UAPK'],
                                                      $ina_varsass_list['PATTERN_ID'],
                                                      $ina_varsass_list['MODULE_VARS_LINK_ID'],
                                                      $ina_varsass_list['MEMBER_VARS'],
    ...
    function chkKeyTypeColValue($in_col_val,$in_table_name,$in_row_id,$in_menu_title){
        ...
        if(strlen($in_col_val)==0){
            ...
            if ( $log_level === 'DEBUG' ){
                ...
                $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207260",
                                     array($lva_table_nameTOid_list[$in_table_name],$in_row_id,$in_menu_title));
                LocalLogPrint(basename(__FILE__),__LINE__,$FREE_LOG);
            }
    ...
    function getIFInfoDB(&$ina_if_info,&$in_error_msg)
    {
        ...
        // SQL作成
        $sql = "SELECT * FROM B_TERRAFORM_CLI_IF_INFO WHERE DISUSE_FLAG = '0'";
        ...
        if( $num_of_rows === 0 ){
            if ( $log_level === 'DEBUG' ){
                ...
                $msgstr = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207270");
                $in_error_msg  = $msgstr;
                LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
            }
            unset($objQuery);
    ...
    function getMasterAccessAuth(&$lva_OpeAccessAuth_list,&$lva_PatternAccessAuth_list) {
        ...
        foreach($sqlAry as $no=>$sql) {
            ...
            $objQuery = $objDBCA->sqlPrepare($sql);
            if( $objQuery->getStatus()===false ){
                $msgstr = $objMTS->getSomeMessage("ITATERRAFORMCLI-ERR-207010",array(basename(__FILE__),__LINE__)); //DBアクセス異常が発生しました。(file:{}line:{})
                $in_error_msg  = $msgstr;
                LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
                LocalLogPrint(basename(__FILE__),__LINE__,$sql);
                LocalLogPrint(basename(__FILE__),__LINE__,$objQuery->getLastError());
