...
//
    ...
    //                                        ['SENSITIVE_FLAG']=>$in_sensitive_flag);
    //
    //  F0001  readValAssDB
    ...
    function addStg1VarsAssDB($ina_varsass_list,&$in_VarsAssignRecodes) {
        ...
        if( ! isset($in_VarsAssignRecodes[$key])) {
            ...
            //Sensitive設定が変更になっているか判断する。
            $sensitive_flag_change = false;
            if($tgt_row["SENSITIVE_FLAG"] == $ina_varsass_list["SENSITIVE_FLAG"]){
                //同一なので具体値の変更チェックへ
                $sensitive_flag_change = false;
            }else{
                //変更があるので具体値の変更チェックを飛ばして最終更新者チェックへ
                $sensitive_flag_change = true;
            }

            if($sensitive_flag_change == false){
                // 具体値が変更になっているか判定する。
                if($tgt_row["SENSITIVE_FLAG"] == 2){
                    //SENSITIVE_FLAGが2(ON)なら、暗号化した値を比較
                    if($tgt_row["VARS_ENTRY"]  == ky_encrypt($ina_varsass_list['VARS_ENTRY'])){
                        // トレースメッセージ
                        if ( $log_level === 'DEBUG' )
                        {
                    ...
                    //SENSITIVE_FLAGが1(OFF)なら、そのままの値を比較
        ...
        if($action == "UPDATE"){
            ...
            //Sensitive設定が2(ON)の場合は暗号化
            if($ina_varsass_list["SENSITIVE_FLAG"] == 2){
                $tgt_row["VARS_ENTRY"]   = ky_encrypt($ina_varsass_list['VARS_ENTRY']);
            }else{
    ...
    function chkValueTypeColValue($in_col_val,
        ...
        if(strlen($in_col_val)>8192){
            // トレースメッセージ
            if ( $log_level === 'DEBUG' ){
                //紐付対象メニューの具体値が規定値(最大 8192 バイト)を超えています。このレコードを処理対象外とします。(MENU_ID:{} 紐付対象メニュー 項番:{} 項目名:{})
                $FREE_LOG = $objMTS->getSomeMessage("ITATERRAFORM-ERR-171220",
                                     array($lva_table_nameTOid_list[$in_table_name],$in_row_id,$in_menu_title));
                LocalLogPrint(basename(__FILE__),__LINE__,$FREE_LOG);
            }
