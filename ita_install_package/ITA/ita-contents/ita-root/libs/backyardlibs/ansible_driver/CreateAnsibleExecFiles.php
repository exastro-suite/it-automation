...
//  F0005-2-1 CreatePioneerTemplateHostvarsfile
...
//  F0011-1 CheckLegacyPlaybookfiles
//  F0011-2 CheckVariablesDefinedInDeviceList
...
class CreateAnsibleExecFiles {
    ...
    const LC_ANS_PIONEER_TEMPLATE_HOST_VARS_DIR  = "pioneer_template_host_vars";
    ...
    const LC_ANS_SSHAGENTCONFIG_FILE         = ".sshAgentConfig.txt";

    const LC_ANS_HOST_VAR_FILE_MK            = "%s/%s";
    const LC_ANS_PIONEER_TEMPLATE_HOST_VAR_FILE_MK  = "%s/%s";
    ...
    private $lv_Ansible_pioneer_template_hosts_vars_Dir;  //pioneer template用 host_varsディレクトリ
    ...
    private  $lv_tpf_vars_list;
    private  $lv_cpf_vars_list;
    private  $lv_use_gbl_vars_list;
    private  $lv_parent_vars_list;
    private  $lv_pioneer_template_parent_vars_list;
    ...
    function __construct($in_driver_id, 
        ...
        $this->lv_pioneer_template_parent_vars_list = array();
...
// #280
        ...
        if ($this->getAnsibleDriverID() == DF_PIONEER_DRIVER_ID){
            ...
            // pioneer_template_host_varsディレクトリ名を記憶
            $this->setAnsible_pioneer_template_hosts_vars_Dir($c_dirwk);
        }
        ...
        $this->setTemporary_file_Dir($c_tmpdir);
                                       ...
                                       $in_exec_option)
    {
        ...
        //////////////////////////////////////////////
        $ret = $this->CreateDirectoryForCollectionProcess($ina_hostinfolist,$ina_host_vars,$ina_pioneer_template_host_vars);
        if($ret === false) {
            return false;
        }
        ...
        case DF_PIONEER_DRIVER_ID:
            ...
            if ( $this->CreateHostvarsfiles($ina_host_vars,$ina_pioneer_template_host_vars,$ina_hostprotcollist,
        ...
        case DF_LEGACY_DRIVER_ID:
            ...
            // 機器一覧の変数定義チェック                  //
            /////////////////////////////////////////////////
            if ( $this->CheckVariablesDefinedInDeviceList($ina_hostinfolist,$ina_host_vars) === false) {
                return false;
            }
            ...
            // テンプレート変数の情報を出力する。
            ////////////////////////////////////////////////
            if( $this->CommitHostVarsfiles($ina_hosts,$ina_hostprotcollist,$ina_host_vars,$ina_pioneer_template_host_vars) === false){
                return false;
            }
            break;
...
//                    $file_id = 0; //ssh秘密鍵
//                    $ret = $this->makeAnsibleVaultSSHFile($this->getAnsibleExecuteUser(),
//                                                          $file_id,
//                                                          $ssh_key_file_path,
//                                                          $ina_hostinfolist[$host_name]['VAULT_CONN_SSH_KEY_FILE'],
//                                                          $ina_hostinfolist[$host_name]['SYSTEM_ID']);
//                    if($ret === false){
//                        return false;
//                    }
...
//                                                          $file_id,
//                                                          $win_ca_file_path,
//                                                          $ina_hostinfolist[$host_name]['VAULT_WINRM_SSL_CA_FILE'],
//                                                          $ina_hostinfolist[$host_name]['SYSTEM_ID']);
//                    if($ret === false){
//                        return false;
//                    }
        ...
        ///////////////////////////////////////////////////
        if($CreateSSHAgentConfigInfoFile === true) {
    ...
    function CreateHostvarsfiles($ina_host_vars,$ina_pioneer_template_host_vars,$ina_hostprotcollist,
                                 $ina_host_child_vars,$ina_DB_child_vars_master) 
    {
        ...
        foreach( $host_list as $host_name){
            ...
            $vars_list = array();
            if(@count($ina_host_vars[$host_name]) != 0) {
                $vars_list = $ina_host_vars[$host_name];
            }

            // ホスト変数定義ファイル作成
            if($this->CreateHostvarsfile("VAR",$hostname,
                                         $file_name,$vars_list) === false)
            {
                return false;
            }
            /////////////////////////////////////////
            ...
            switch($this->getAnsibleDriverID()){
            case DF_PIONEER_DRIVER_ID:
                $pioneer_template_vars_list = array();
                if(@count($ina_pioneer_template_host_vars[$host_name]) != 0) {
                    $pioneer_template_vars_list = $ina_pioneer_template_host_vars[$host_name];
                }
                $file_name = $this->getAnsible_pioneer_template_host_var_file($host_vars_file);
                if($this->CreatePioneerTemplateHostvarsfile("VAR",$hostname,
                                                            $file_name,$pioneer_template_vars_list) === false)
                {
                    return false;
                }
                break;
    ...
    // F0005-2-1
    // 処理内容
    //   Pioneer template用 ホスト変数定義ファイル(1ホスト)を作成する。
    // パラメータ
    //   $in_var_type:      登録対象の変数タイプ
    //                      "VAR"/"CPF"/"TPF"/"CMT"
    //   $in_host_name:     ホスト名
    ...
    //   false:  異常
    ////////////////////////////////////////////////////////////////////////////////
    function CreatePioneerTemplateHostvarsfile($in_var_type,$in_host_name,
                                               $in_file_name,$ina_var_list,
                                               $in_mode="w"){
        if($this->getAnsibleDriverID() != DF_PIONEER_DRIVER_ID) {
            return false;
        ...
        if($in_var_type != "CMT"){
            foreach( $ina_var_list as $var=>$val ){
                if(@count($this->lv_pioneer_template_parent_vars_list[$in_host_name][$var]) != 0){
                    continue;
                }
                $this->lv_pioneer_template_parent_vars_list[$in_host_name][$var] = 0;
 ...
 $ina_host_vars[$host_name][self::LC_ANS_LOGINHOST_VAR_NAME];
            ...
            case DF_PIONEER_DRIVER_ID:
                $file_name = $this->getAnsible_pioneer_template_host_var_file($host_vars_file);
                if($this->CreatePioneerTemplateHostvarsfile("SYS_VAR",$hostname,$file_name,$host_vars_list,"a") === false)
                {
                    return false;
                }
                break;
            ...
            foreach( $varsArray as $var_name ){ 
                ...
                if(@strlen($ina_host_vars[$host_name][$var_name])==0) {
                    if($var_name == self::LC_ANS_PROTOCOL_VAR_NAME){
                        $msgstr = $this->lv_objMTS->getSomeMessage("ITAANSIBLEH-ERR-56284",
                                                                    array($var_name, $host_name));
                        $this->LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
                    }
                    elseif($var_name == self::LC_ANS_USERNAME_VAR_NAME){
                        $msgstr = $this->lv_objMTS->getSomeMessage("ITAANSIBLEH-ERR-56282",
                                                                    array($var_name, $host_name));
                        $this->LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
                    }
                    elseif($var_name == self::LC_ANS_PASSWD_VAR_NAME){
                        $msgstr = $this->lv_objMTS->getSomeMessage("ITAANSIBLEH-ERR-56283",
                                                                    array($var_name, $host_name));
                        $this->LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
                    }
                    elseif($var_name == self::LC_ANS_LOGINHOST_VAR_NAME){
                        $msgstr = $this->lv_objMTS->getSomeMessage("ITAANSIBLEH-ERR-56281",
                                                                    array( $var_name, $host_name));
                        $this->LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
                    }
                    else{
                        ...
                        $msgstr = $this->lv_objMTS->getSomeMessage("ITAANSIBLEH-ERR-55285",
                                                                    array( $var_name, $host_name)); 
                        $this->LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
                    }
                    // エラーリターンする
            ...
            foreach( $playbook_list as $playbookpkey=>$playbook ){
                ...
                if(count($file_global_vars_list) != 0){
                    ...
                    foreach( $file_global_vars_list as $var_name ){
                        if(array_key_exists($var_name,$this->lva_global_vars_list) === false){
                ...
                $FillterVars       = true;  // Fillterを含む変数の抜き出しあり
                SimpleFillterVerSearch(DF_HOST_VAR_HED,$dataString,$varsLineArray,$file_vars_list,$local_vars,$FillterVars);
    ...
    function CheckPioneerPlaybookfiles($ina_hosts,$ina_host_vars,$ina_vault_vars,$ina_dialog_files,$ina_hostprotcollist,$in_SpecialVarValReplace=false){
        ...
        foreach( $ina_hosts as $no=>$host_name ){
            ...
            foreach( $dialog_file_list as $includeno=>$pkeylist ){
                foreach( $pkeylist as $playbook_pkey=>$playbook ){
                    ...
                    if($in_SpecialVarValReplace === false) {
                        ...
                        SimpleFillterVerSearch(DF_HOST_GBL_HED,$dataString,$varsLineArray,$varsArray,$local_vars,$FillterVars);
                    ...
                    $varsArray     = array();
                    $FillterVars   = false;  // Fillterを含む変数の抜き出しあり
                    SimpleFillterVerSearch(DF_HOST_CPF_HED,$dataString,$varsLineArray,$varsArray,$local_vars,$FillterVars);
                    $file_copy_vars_list = $varsArray;
			...
			//rary[6000074] = "ホスト変数ファイルがYAML形式で生成されていません。(ホスト名:{})";
                    ...
                    $varsArray     = array();
                    $FillterVars   = false;
                    SimpleFillterVerSearch(DF_HOST_TPF_HED,$dataString,$varsLineArray,$varsArray,$local_vars,$FillterVars);
                    ...
                    if($in_SpecialVarValReplace === false) {
                        ...
                        $varsArray     = array();
                        $FillterVars   = false;  // Fillterを含む変数の抜き出しあり
                        SimpleFillterVerSearch(DF_HOST_VAR_HED,$dataString,$varsLineArray,$varsArray,$local_vars,$FillterVars);
                        $file_vars_list = $varsArray;
                    ...
                    if(isset($objWSRA) === false) {
                        // WrappedStringReplaceAdminクラス生成のみ
                        $local_vars = array();
...
// 作業パターンの出力結果から特定の文字列でOK/NGを判断するコマンド(command)の処理追加
    ...
    ////////////////////////////////////////////////////////////////////////////////
    // 処理内容
    //  pioneer template用 host_varsディレクトリ名を記憶
    //   暗号化された変数のみのホスト変数ファイル
    // パラメータ
    //   $in_dir:      pioneer_template_hosts_varsディレクトリ
    // 
    // 戻り値
    //   なし
    ////////////////////////////////////////////////////////////////////////////////
    function setAnsible_pioneer_template_hosts_vars_Dir($in_indir){
        $this->lv_Ansible_pioneer_template_hosts_vars_Dir = $in_indir;
    }
    ////////////////////////////////////////////////////////////////////////////////
    // 処理内容
    //   pioneer template用 host_varsディレクトリ名を取得
    // パラメータ
    //   なし
    // 
    // 戻り値
    //   vault_host_varsディレクトリ名
    ////////////////////////////////////////////////////////////////////////////////
    function getAnsible_pioneer_template_hosts_vars_Dir(){
        return($this->lv_Ansible_pioneer_template_hosts_vars_Dir);
    }
    ...
    ////////////////////////////////////////////////////////////////////////////////
    // 処理内容
    //   Pioneer template用 ホスト変数定義ファイル名を取得
    // パラメータ
    ...
    //   ホスト変数定義ファイル名
    ////////////////////////////////////////////////////////////////////////////////
    function getAnsible_pioneer_template_host_var_file($in_hostname){
        $file = sprintf(self::LC_ANS_PIONEER_TEMPLATE_HOST_VAR_FILE_MK,$this->getAnsible_pioneer_template_hosts_vars_Dir(),$in_hostname);
        return($file);
    }
        ...
        while ( $row = $objQuery->resultFetch() ){
            if($row['DISUSE_FLAG']=='0'){
                ...
                if($row['SSH_KEY_FILE_PASSPHRASE'] != "") {
                    $ina_hostinfolist[$row['IP_ADDRESS']]['SSH_KEY_FILE_PASSPHRASE'] = ky_decrypt($row['SSH_KEY_FILE_PASSPHRASE']); 
                } else {
...
// ansibleがvaultで暗号化された秘密鍵ファイルに対応していないので
// 対応したタイミングで秘密鍵ファイルをvaultで暗号化
//                $ina_hostinfolist[$row['IP_ADDRESS']]['VAULT_CONN_SSH_KEY_FILE'] = $row['VAULT_CONN_SSH_KEY_FILE']; 
//                $ina_hostinfolist[$row['IP_ADDRESS']]['VAULT_WINRM_SSL_CA_FILE'] = $row['VAULT_WINRM_SSL_CA_FILE']; 
    ...
    //   $in_pattern_id:        作業パターンID
    //   $in_operation_id:      オペレーションID
    //   $ina_host_vars:        変数一覧返却配列 (pionner:passwordColumnの具体値がky_encryptで暗号化)
    //                          [ホスト名(IP)][ 変数名 ]=>具体値
    //   $ina_pionner_template_host_vars: pioneer template用 変数一覧返却配列 (passwordColumnの具体値がansible-vaultで暗号化)
    //                          [ホスト名(IP)][ 変数名 ]=>具体値
    //   $ina_vault_vars:       PasswordCoulumn変数一覧(Pioneer用)
    //                          [ 変数名 ] = << 変数名 >>
    //   $ina_vault_host_vars_file_list:  PasswordCoulumn変数のみのホスト変数一覧(Pioneer用)
...
// 代入値管理のみあるホスト変数(作業対象ホストにない)はじくためのSELECT追加
        ...
        $ina_host_vars = array();
        $ina_pionner_template_host_vars = array();
        ...
        foreach( $tgt_row as $row )
        {
            // ina_pionner_template_host_vars用の具体値
            ...
            if($row['DISUSE_FLAG']=='0'){
                ...
                if($row['PHO_LINK_HOST_COUNT'] > 0){
                    ...
                    if($row['SENSITIVE_FLAG'] == DF_SENSITIVE_ON) {
                        ...
                        $row['VARS_ENTRY_PIONEER_TEMP'] = $this->makeMultilineValue($row['VARS_ENTRY_PIONEER_TEMP']);
                        //-----複数行具体値の場合に複数行の扱い記号を付ける
                    }
                    // ----具体値の暗号化が必要か判定
                    ...
                    if(($row['VARS_NAME_COUNT'] == 1) && (@strlen($row['ASSIGN_SEQ']) == 0)) {
                        ...
                        $ina_pionner_template_host_vars[$row['IP_ADDRESS']][$row['VARS_NAME']]= $row['VARS_ENTRY_PIONEER_TEMP'];
                        ...
                        if(@count($ina_pionner_template_host_vars[$row['IP_ADDRESS']][$row['VARS_NAME']])==0){
                            $ina_pionner_template_host_vars[$row['IP_ADDRESS']][$row['VARS_NAME']] = "";
                        }
                        ...
                        $value_count = $this->ArrayTypeValue_encode($ina_pionner_template_host_vars[$row['IP_ADDRESS']][$row['VARS_NAME']],$row['VARS_ENTRY_PIONEER_TEMP']);
...
;
    ...
    // パラメータ
    //   $in_dir:      一時ファイル格納ディレクトリ
    //
    // 戻り値
    //   なし
    ////////////////////////////////////////////////////////////////////////////////
    function setTemporary_file_Dir($in_indir){
        $this->lv_Ansible_temporary_files_Dir = $in_indir;
    }
    ...
    //   一時ファイル格納ディレクトリ名
    ////////////////////////////////////////////////////////////////////////////////
    function getTemporary_file_Dir(){
        return($this->lv_Ansible_temporary_files_Dir);
    }
    ...
    function CheckTemplatefile($ina_hosts,$ina_host_vars,$in_child_playbook,$in_tpf_key,$in_tpf_file_name,
        ...
        // 未登録のグローバル変数がある場合は別のエラーとして扱われるのでリターンする
        if($result_code === false) {
            return false;
        }
...
//        }
    ...
    //   false:  異常
    ////////////////////////////////////////////////////////////////////////////////
    function CommitHostVarsfiles($ina_hosts,$ina_hostprotcollist,$ina_host_vars,$ina_pioneer_template_host_vars){
        // ホスト変数配列のホスト)分繰返し
        foreach( $ina_hosts as $host_name){
            foreach($ina_hostprotcollist[$host_name] as $hostname=>$prolist)
            $host_vars_file = $hostname;
            ...
            case DF_PIONEER_DRIVER_ID:
                ...
                if($this->CreatePioneerTemplateHostvarsfile("CMT",$hostname,$file_name,$pioneer_template_vars_list,"a") === false){
                    return false;
                }
                break;
            }
    ...
    function CreatePioneerCopyFiles($ina_hosts,$ina_dialog_files,$ina_hostprotcollist,$ina_host_vars){
        ...
        foreach( $ina_hosts as $no=>$host_name ){
            ...
            if ( count($la_cpf_path) > 0 ){
                ...
                /////////////////////////////////////////
                $file_name = $this->getAnsible_pioneer_template_host_var_file($host_vars_file);
                if($this->CreatePioneerTemplateHostvarsfile("CPF",$hostname,$file_name,$la_cpf_path,"a") === false)
                {
                    return false;
                }
            }
    ...
    function CreatePioneertemplatefiles($ina_hosts,$ina_dialog_files,$ina_hostprotcollist,$ina_host_vars) {
        ...
        foreach( $ina_hosts as $no=>$host_name ) {
            ...
            if ( count($la_tpf_path) > 0 ) {
                ...
                /////////////////////////////////////////
                $file_name = $this->getAnsible_pioneer_template_host_var_file($host_vars_file);
                if($this->CreatePioneerTemplateHostvarsfile("TPF",$hostname,$file_name,$la_tpf_path,"a") === false) {
                    return false;
                }
            }
        }
...
// ansibleがvaultで暗号化された秘密鍵ファイルに対応していないので
// 対応したタイミングで秘密鍵ファイルをvaultで暗号化
    //$in_file_id 0: ssh秘密鍵/1: WinRM
    function makeAnsibleVaultSSHFile($in_exec_user,$in_file_id,$in_sshKeyFile,$in_vault_sshKeyFileData,$in_system_id) {
        // 暗号化されていない
        global      $root_dir_path;
        global      $vg_OrchestratorSubId;
        $vaultobj = new AnsibleVault();
        ...
        // 鍵ファイルが暗号化されているか判定
        if(strlen(trim($in_vault_sshKeyFileData)) != 0) {
            $vaultData = $in_vault_sshKeyFileData;
            $ret = file_put_contents($in_sshKeyFile,$vaultData);
            if($ret === false) {
                $msgstr = $this->lv_objMTS->getSomeMessage("ITAANSIBLEH-ERR-6000116",array($in_system_id));
                $this->LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
                return false;
            ...
            // 鍵ファイルのスクランブルを復号
            $tmp_decrypt_file = sprintf("/tmp/%s_%s",getmypid(),basename($in_sshKeyFile));
            $ret = ky_file_decrypt($in_sshKeyFile,$tmp_decrypt_file);
            if($ret === false) {
                $msgstr = $this->lv_objMTS->getSomeMessage("ITAANSIBLEH-ERR-6000116",array($in_system_id));
                $this->LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
                return false;
            ...
            // 鍵ファイルの内容を取得
            $decryptData = file_get_contents($tmp_decrypt_file);
            ...
            $RequestURI = "/restapi/ansible_driver/file_vault.php";
            $Method     = 'GET';
            $RequestContents
                = array(
                    //データリレイパス(不要だがI/Fを合わせる為にダミー値を設定)
            ...
            $aryForBind = array('VAULT_DATA'=>$vaultData);
            // Vaultで暗号かした内容をDBに保持
            if($in_file_id == 0) {
                $strQuery =   "UPDATE C_STM_LIST SET VAULT_CONN_SSH_KEY_FILE = :VAULT_DATA "
                            . "WHERE SYSTEM_ID = " . $in_system_id;
            } else {
                $strQuery =   "UPDATE C_STM_LIST SET VAULT_WINRM_SSL_CA_FILE = :VAULT_DATA "
                            . "WHERE SYSTEM_ID = " . $in_system_id;
            }  
...
// -----今後の拡張処理
    ...
    //   false: 異常
    ////////////////////////////////////////////////////////////////////////////////
    function CreateDirectoryForCollectionProcess($ina_hostinfolist,&$ina_host_vars,&$ina_pioneer_template_host_vars) {
    ...
    //   false: 異常
    ////////////////////////////////////////////////////////////////////////////////
    function CreateOperationVariables($in_operation_id,$ina_hostinfolist,&$ina_host_vars,&$ina_pioneer_template_host_vars) {
        $sql = " SELECT \n"
             . "   OPERATION_NAME, \n"
    ...
    //   false: 異常
    ////////////////////////////////////////////////////////////////////////////////
    function CreateSSHAgentConfigInfoFile($file,$hostname,$ssh_key_file,$pssphrase) {
        $row = sprintf("%s\t%s\t%s\t\n",$hostname,$ssh_key_file,$pssphrase);
        $ret = file_put_contents($file,$row,FILE_APPEND);
        if($ret === false) {
            $msgstr = $this->lv_objMTS->getSomeMessage("ITAANSIBLEH-ERR-70082");
            $this->LocalLogPrint(basename(__FILE__),__LINE__,$msgstr);
            return false;
