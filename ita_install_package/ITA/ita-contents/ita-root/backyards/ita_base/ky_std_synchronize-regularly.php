...
//
    ...
    try{
        ...
        foreach($aryCreateFirstNextDate as $rowOfReguralyList){
            ...
            $aryRetBody = getSequenceLockInTrz('C_REGULARLY_LIST_JSQ','A_SEQUENCE');
            if( $aryRetBody[1] != 0 ){
                // 例外処理へ
            ...
            // ----履歴シーケンス払い出し
            $aryRetBody = getSequenceValueFromTable('C_REGULARLY_LIST_JSQ', 'A_SEQUENCE', FALSE );
            if( $aryRetBody[1] != 0 ){
                // 例外処理へ
        ...
        foreach($aryExecutionSoonNextDate as $rowOfReguralyList){
            ...
            if(strtotime($nextExecutionDate) >= strtotime($now)){
                ...
                if($beforeExecuteCheckFlag == false){
                    ...
                    $retArray = $objOLA->registerSymphonyInstance($symphonyClassNo, $opertionNoIdbh, $nextExecutionDate, $aryOptionOrder, $aryOptionOrderOverride, $executionUserId, $executionUserName);
