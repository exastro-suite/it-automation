...
if [ "${RAS_FILE_PASSWD}" != "undefine" ]; then
  ...
  if [ ${EXIT_CODE}  -ne 0 ]; then
    ...
    if [ ${EXIT_CODE}  -eq 200 ]; then
      echo "Bad passphrase, Please check the Ansible Tower(Ansible Automation Controller) host list. (host:${HOST})" >> /dev/stderr
    # passphrase不要
    elif [ ${EXIT_CODE}  -eq 201 ]; then
      echo "No passphrase requireds, Please check the Ansible Tower(Ansible Automation Controller) host list. (host:${HOST})" >> /dev/stderr
    else
      echo "Bad private key file(id_ras) or passphrase, Please check the Ansible Tower(Ansible Automation Controller) host list. (host:${HOST})" >> /dev/stderr
    fi
