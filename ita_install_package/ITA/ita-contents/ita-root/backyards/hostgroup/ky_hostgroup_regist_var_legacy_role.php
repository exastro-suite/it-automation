...
try{
    ...
    //////////////////////////
    $hgVarLinkLegacyRoleTable = new HgVarLinkLegacyRoleTable($objDBCA, $db_model_ch);
    $sql = $hgVarLinkLegacyRoleTable->createSselect("WHERE DISUSE_FLAG = '0'");

    // SQL実行
    $result = $hgVarLinkLegacyRoleTable->selectTable($sql);
    if(!is_array($result)){
...
function registVarsMaster(&$hostGrpVarLinkArray) {
    ...
    $insertCnt = 0;       // 登録件数
    $disuseCnt = 0;       // 廃止件数
    $ansibleLrlVarsMasterTable = new AnsibleLrlVarsMasterTable($objDBCA, $db_model_ch);

    try{
        //////////////////////////
        // 変数名一覧テーブルを検索
        //////////////////////////
        $sql = $ansibleLrlVarsMasterTable->createSselect("");

        // SQL実行
        $result = $ansibleLrlVarsMasterTable->selectTable($sql);
        if(!is_array($result)){
            $msg = $objMTS->getSomeMessage('ITAHOSTGROUP-ERR-5001', $result);
            outputLog($msg);
            throw new Exception($msg);
        ...
        foreach($hostGrpVarLinkArray as $hostGrpVarLink) {
            ...
            if(true === $matchFlg) {
                ...
                if("1" === $varsMaster['DISUSE_FLAG']) {
                    ...
                    // 復活する
                    $updateData['VARS_ATTRIBUTE_01']    = 2;                            // 変数タイプ
                    ...
                    // 変数名一覧テーブルを更新
                    //////////////////////////
                    $result = $ansibleLrlVarsMasterTable->updateTable($updateData, $jnlSeqNo);
                    if(true !== $result){
                        $msg = $objMTS->getSomeMessage('ITAHOSTGROUP-ERR-5001', $result);
                        outputLog($msg);
                        throw new Exception($msg);
            ...
            // 変数名が一致するデータがない場合
                ...
                $insertData['VARS_ATTRIBUTE_01']    = 2;                            // 変数タイプ
                ...
                // 変数名一覧テーブルに登録
                //////////////////////////
                $result = $ansibleLrlVarsMasterTable->insertTable($insertData, $seqNo, $jnlSeqNo);
                if(true !== $result){
                    $msg = $objMTS->getSomeMessage('ITAHOSTGROUP-ERR-5001', $result);
                    outputLog($msg);
                    throw new Exception($msg);
        ...
        foreach($varsMasterArray as $varsMaster) {
            ...
            // 変数名一覧テーブルを更新
            //////////////////////////
            $result = $ansibleLrlVarsMasterTable->updateTable($disuseData, $jnlSeqNo);
            if(true !== $result){
                $msg = $objMTS->getSomeMessage('ITAHOSTGROUP-ERR-5001', $result);
                outputLog($msg);
                throw new Exception($msg);
        ...
        // 変数名一覧テーブルを検索
        //////////////////////////
        $sql = $ansibleLrlVarsMasterTable->createSselect("WHERE DISUSE_FLAG = '0'");
        ...
        if(LOG_LEVEL === 'DEBUG'){
            // 件数ログ出力
            outputLog($objMTS->getSomeMessage('ITAHOSTGROUP-STD-10004', array($ansibleLrlVarsMasterTable->tableName, $insertCnt, $updateCnt, $disuseCnt)));
        }
    ...
    $insertCnt = 0;       // 登録件数
    $disuseCnt = 0;       // 廃止件数
    $ansLrlPtnVarsLinkTable = new AnsLrlPtnVarsLinkTable($objDBCA, $db_model_ch);

    try{
        //////////////////////////
        // Movement変数紐付管理テーブルを検索
        //////////////////////////
        $sql = $ansLrlPtnVarsLinkTable->createSselect("");

        // SQL実行
        $result = $ansLrlPtnVarsLinkTable->selectTable($sql);
        if(!is_array($result)){
            $msg = $objMTS->getSomeMessage('ITAHOSTGROUP-ERR-5001', $result);
            outputLog($msg);
            throw new Exception($msg);
        ...
        foreach($hostGrpVarLinkArray as $hostGrpVarLink) {
            ...
            if(true === $matchFlg) {
                ...
                if("1" === $patternVarsLinkMaster['DISUSE_FLAG']) {
                    ...
                    //////////////////////////
                    $result = $ansLrlPtnVarsLinkTable->updateTable($updateData, $jnlSeqNo);
                    if(true !== $result){
                        $msg = $objMTS->getSomeMessage('ITAHOSTGROUP-ERR-5001', $result);
                        outputLog($msg);
                        throw new Exception($msg);
            ...
            // Movement、変数名が一致するデータがない場合
                ...
                //////////////////////////
                $result = $ansLrlPtnVarsLinkTable->insertTable($insertData, $seqNo, $jnlSeqNo);
                if(true !== $result){
                    $msg = $objMTS->getSomeMessage('ITAHOSTGROUP-ERR-5001', $result);
                    outputLog($msg);
                    throw new Exception($msg);
        ...
        foreach($patternVarsLinkMasterArray as $patternVarsLinkMaster) {
            ...
            // Movement変数紐付管理テーブルを更新
            //////////////////////////
            $result = $ansLrlPtnVarsLinkTable->updateTable($disuseData, $jnlSeqNo);
            if(true !== $result){
                $msg = $objMTS->getSomeMessage('ITAHOSTGROUP-ERR-5001', $result);
                outputLog($msg);
                throw new Exception($msg);
        ...
        // Movement変数紐付管理テーブルを検索
        //////////////////////////
        $sql = $ansLrlPtnVarsLinkTable->createSselect("WHERE DISUSE_FLAG = '0'");
        ...
        if(LOG_LEVEL === 'DEBUG'){
            // 件数ログ出力
            outputLog($objMTS->getSomeMessage('ITAHOSTGROUP-STD-10004', array($ansLrlPtnVarsLinkTable->tableName, $insertCnt, $updateCnt, $disuseCnt)));
        }
...
function registPatternHostOpLinkMaster(&$hostGrpVarLinkArray) {
    ...
    $insertCnt = 0;       // 登録件数
    $disuseCnt = 0;       // 廃止件数
    $ansibleLrlPhoLinkTable = new AnsibleLrlPhoLinkTable($objDBCA, $db_model_ch);

    try{
        //////////////////////////
        // 作業対象ホストテーブルを検索
        //////////////////////////
        $sql = $ansibleLrlPhoLinkTable->createSselect("");

        // SQL実行
        $result = $ansibleLrlPhoLinkTable->selectTable($sql);
        if(!is_array($result)){
            $msg = $objMTS->getSomeMessage('ITAHOSTGROUP-ERR-5001', $result);
            outputLog($msg);
            throw new Exception($msg);
        ...
        foreach($hostGrpVarLinkArray as $hostGrpVarLink) {
            ...
            if(true === $matchFlg) {
                ...
                if("1" === $patternHostOpLinkMaster['DISUSE_FLAG']) {
                    ...
                    // 作業対象ホストテーブルを更新
                    //////////////////////////
                    $result = $ansibleLrlPhoLinkTable->updateTable($updateData, $jnlSeqNo);
                    if(true !== $result){
                        $msg = $objMTS->getSomeMessage('ITAHOSTGROUP-ERR-5001', $result);
                        outputLog($msg);
                        throw new Exception($msg);
            ...
            // オペレーション、Movement、ホストが一致するデータがない場合
                ...
                // 作業対象ホストテーブルに登録
                //////////////////////////
                $result = $ansibleLrlPhoLinkTable->insertTable($insertData, $seqNo, $jnlSeqNo);
                if(true !== $result){
                    $msg = $objMTS->getSomeMessage('ITAHOSTGROUP-ERR-5001', $result);
                    outputLog($msg);
                    throw new Exception($msg);
        ...
        foreach($patternHostOpLinkMasterArray as $patternHostOpLinkMaster) {
            ...
            // 作業対象ホストテーブルを更新
            //////////////////////////
            $result = $ansibleLrlPhoLinkTable->updateTable($disuseData, $jnlSeqNo);
            if(true !== $result){
                $msg = $objMTS->getSomeMessage('ITAHOSTGROUP-ERR-5001', $result);
                outputLog($msg);
                throw new Exception($msg);
        ...
        // 作業対象ホストテーブルを検索
        //////////////////////////
        $sql = $ansibleLrlPhoLinkTable->createSselect("WHERE DISUSE_FLAG = '0'");
        ...
        if(LOG_LEVEL === 'DEBUG'){
            // 件数ログ出力
            outputLog($objMTS->getSomeMessage('ITAHOSTGROUP-STD-10004', array($ansibleLrlPhoLinkTable->tableName, $insertCnt, $updateCnt, $disuseCnt)));
        }
...
function registVarsAssignMaster($hostGrpVarLinkArray) {
    ...
    $disuseCnt = 0;       // 廃止件数
    $linkDataArray = array();
    $ansibleLrlVarsAssignTable = new AnsibleLrlVarsAssignTable($objDBCA, $db_model_ch);
    ...
    try{
        ...
        // 代入地管理テーブルを検索
        //////////////////////////
        $sql = $ansibleLrlVarsAssignTable->createSselect("");

        // SQL実行
        $result = $ansibleLrlVarsAssignTable->selectTable($sql);
        if(!is_array($result)){
            $msg = $objMTS->getSomeMessage('ITAHOSTGROUP-ERR-5001', $result);
            outputLog($msg);
            throw new Exception($msg);
        ...
        foreach($linkDataArray as $key => $linkData) {
            ...
            if(true === $matchFlg) {
                ...
                if("1" === $varsAssignMaster['DISUSE_FLAG']) {
                   ...
                   if(array_key_exists($key, $maxAssignSeqArray)) {
                    ...
                    // 代入値管理テーブルを更新
                    //////////////////////////
                    $result = $ansibleLrlVarsAssignTable->updateTable($updateData, $jnlSeqNo);
                    if(true !== $result){
                        $msg = $objMTS->getSomeMessage('ITAHOSTGROUP-ERR-5001', $result);
                        outputLog($msg);
                        throw new Exception($msg);
            ...
            // オペレーション、Movement、ホスト、変数名、具体値が一致するデータがない場合
               ...
               if(array_key_exists($key, $maxAssignSeqArray)) {
                ...
                // 代入値管理テーブルに登録
                //////////////////////////
                $result = $ansibleLrlVarsAssignTable->insertTable($insertData, $seqNo, $jnlSeqNo);
                if(true !== $result){
                    $msg = $objMTS->getSomeMessage('ITAHOSTGROUP-ERR-5001', $result);
                    outputLog($msg);
                    throw new Exception($msg);
        ...
        foreach($varsAssignMasterArray as $varsAssignMaster) {
            ...
            // 代入値管理テーブルを更新
            //////////////////////////
            $result = $ansibleLrlVarsAssignTable->updateTable($disuseData, $jnlSeqNo);
            if(true !== $result){
                $msg = $objMTS->getSomeMessage('ITAHOSTGROUP-ERR-5001', $result);
                outputLog($msg);
                throw new Exception($msg);
        ...
        if(LOG_LEVEL === 'DEBUG'){
            // 件数ログ出力
            outputLog($objMTS->getSomeMessage('ITAHOSTGROUP-STD-10004', array($ansibleLrlVarsAssignTable->tableName, $insertCnt, $updateCnt, $disuseCnt)));
        }
