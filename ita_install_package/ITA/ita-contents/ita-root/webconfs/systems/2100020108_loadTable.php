...
$tmpFx = function (&$aryVariant=array(),&$arySetting=array()){
    ...
    $arrayWebSetting['page_info'] = $g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-204080");
/*
Ansible(Legacy)作業対象ホスト管理
*/
    $tmpAry = array(
    ...
    $table = new TableControlAgent('B_ANSIBLE_LNS_PHO_LINK','PHO_LINK_ID', $g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-204090"), 'B_ANSIBLE_LNS_PHO_LINK_JNL', $tmpAry);
    $tmpAryColumn = $table->getColumns();
    $tmpAryColumn['PHO_LINK_ID']->setSequenceID('B_ANSIBLE_LNS_PHO_LINK_RIC');
    $tmpAryColumn['JOURNAL_SEQ_NO']->setSequenceID('B_ANSIBLE_LNS_PHO_LINK_JSQ');
    unset($tmpAryColumn);

    // QMファイル名プレフィックス
    $table->setDBMainTableLabel($g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-205010"));
    // エクセルのシート名
    $table->getFormatter('excel')->setGeneValue('sheetNameForEditByFile', $g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-205020"));
    ...
    $c = new IDColumn('OPERATION_NO_UAPK',$g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-205030"),'E_OPERATION_LIST','OPERATION_NO_UAPK','OPERATION','',array('OrderByThirdColumn'=>'OPERATION_NO_UAPK'));
    $c->setDescription($g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-205040"));//エクセル・ヘッダでの説明
