...
$tmpFx = function (&$aryVariant=array(),&$arySetting=array()){
    ...
    $arrayWebSetting['page_info'] =  $g['objMTS']->getSomeMessage('ITABASEH-MNU-307001');
...
*/
    ...
    $table = new TableControlAgent('C_REGULARLY2_LIST','REGULARLY_ID', $g['objMTS']->getSomeMessage('ITABASEH-MNU-307002'), 'C_REGULARLY2_LIST_JNL', $tmpAry);
    $tmpAryColumn = $table->getColumns();
    $tmpAryColumn['REGULARLY_ID']->setSequenceID('C_REGULARLY2_LIST_RIC');
    $tmpAryColumn['JOURNAL_SEQ_NO']->setSequenceID('C_REGULARLY2_LIST_JSQ');
    unset($tmpAryColumn);

    $table->setJsEventNamePrefix(true);

    // QMファイル名プレフィックス
    $table->setDBMainTableLabel($g['objMTS']->getSomeMessage('ITABASEH-MNU-307003'));
    ...
    $c->setJournalDispIDOfMaster('CONDUCTOR_NAME');
    $c->setRequired(true); //登録/更新時には、入力必須
    $table->addColumn($c);

    //オペレーションID
    $c = new IDColumn('OPERATION_NO_IDBH',$g['objMTS']->getSomeMessage('ITABASEH-MNU-307024'), 'C_OPERATION_LIST', 'OPERATION_NO_IDBH', 'OPERATION_NAME', '', array('SELECT_ADD_FOR_ORDER'=>array('OPERATION_NO_UAPK'), 'ORDER'=>'ORDER BY ADD_SELECT_1'));
    $c->setDescription($g['objMTS']->getSomeMessage('ITABASEH-MNU-307025')); //エクセル・ヘッダでの説明
    ...
    $c = new IDColumn('STATUS_ID', $g['objMTS']->getSomeMessage('ITABASEH-MNU-307006'), 'B_REGULARLY_STATUS', 'REGULARLY_STATUS_ID', 'REGULARLY_STATUS_NAME', '');
    $c->setDescription($g['objMTS']->getSomeMessage('ITABASEH-MNU-307007')); //エクセル・ヘッダでの説明
    ...
    $strWebUIText = $g['objMTS']->getSomeMessage('ITABASEH-MNU-307026');
    ...
    $c = new IDColumn('EXECUTION_USER_ID', $g['objMTS']->getSomeMessage('ITABASEH-MNU-307027'), 'D_ACCOUNT_LIST', 'USER_ID', 'USERNAME_JP', '');
    $c->setDescription($g['objMTS']->getSomeMessage('ITABASEH-MNU-307028')); //エクセル・ヘッダでの説明
    $c->setAllowSendFromFile(false); //エクセル/CSVからのアップロードは不可能
    ...
    //スケジュール設定ボタン
    $c = new LinkButtonColumn('', $g['objMTS']->getSomeMessage('ITABASEH-MNU-307008'), $g['objMTS']->getSomeMessage('ITABASEH-MNU-307008'), 'regularlyPWattern', array());
    ...
    //スケジュール
    $cg = new ColumnGroup($g['objMTS']->getSomeMessage('ITABASEH-MNU-307009'));
        //次回実行日付
        $tmpObjFunction = function($objColumn, $strEventKey, &$exeQueryData, &$reqOrgData=array(), &$aryVariant=array()){
            $boolRet = true;
            $intErrorType = null;
        ...
        $c = new DateTimeColumn('NEXT_EXECUTION_DATE', $g['objMTS']->getSomeMessage('ITABASEH-MNU-307010'), 'DATETIME', 'DATETIME', false);
        $c->setDescription($g['objMTS']->getSomeMessage('ITABASEH-MNU-307010'));//エクセル・ヘッダでの説明
        ...
        //開始日付
        $c = new DateTimeColumn('START_DATE', $g['objMTS']->getSomeMessage('ITABASEH-MNU-307011'), 'DATETIME', 'DATETIME', false);
        $c->setDescription($g['objMTS']->getSomeMessage('ITABASEH-MNU-307011'));//エクセル・ヘッダでの説明
        $c->setRequired(true); //登録/更新時には、入力必須
        $c->setSecondsInputOnIU(false); //UI表示時に秒を非表示
        $c->setSecondsInputOnFilter(false); //フィルタ表示時に秒を非表示
        ...
        //終了日付
        $c = new DateTimeColumn('END_DATE', $g['objMTS']->getSomeMessage('ITABASEH-MNU-307012'), 'DATETIME', 'DATETIME', false);
        $c->setDescription($g['objMTS']->getSomeMessage('ITABASEH-MNU-307012'));//エクセル・ヘッダでの説明
        $c->setSecondsInputOnIU(false); //UI表示時に秒を非表示
        $c->setSecondsInputOnFilter(false); //フィルタ表示時に秒を非表示
        $c->setOutputType('update_table', new OutputType(new ReqTabHFmt(), new TextHiddenInputTabBFmt(''))); //テキストと、隠しテキストインプットを出力する
        ...
        //周期
        $c = new IDColumn('REGULARLY_PERIOD_ID', $g['objMTS']->getSomeMessage('ITABASEH-MNU-307013'), 'B_REGULARLY_PERIOD', 'REGULARLY_PERIOD_ID', 'REGULARLY_PERIOD_NAME', '', array('SELECT_ADD_FOR_ORDER'=>array('REGULARLY_PERIOD_ID'), 'ORDER'=>'ORDER BY ADD_SELECT_1'));
        $c->setDescription($g['objMTS']->getSomeMessage('ITABASEH-MNU-307013'));//エクセル・ヘッダでの説明
        $c->setRequired(true);//登録/更新時には、入力必須
        $c->setOutputType('update_table', new OutputType(new ReqTabHFmt(), new TextHiddenInputTabBFmt(''))); //テキストと、隠しテキストインプットを出力する
        ...
        //間隔
        $c = new NumColumn('EXECUTION_INTERVAL', $g['objMTS']->getSomeMessage('ITABASEH-MNU-307014'));
        $c->setDescription($g['objMTS']->getSomeMessage('ITABASEH-MNU-307014'));//エクセル・ヘッダでの説明
        $c->setRequired(true);//登録/更新時には、入力必須
        ...
        //週番号
        $c = new IDColumn('PATTERN_WEEK_NUMBER', $g['objMTS']->getSomeMessage('ITABASEH-MNU-307015'), 'B_WEEK_NUMBER', 'WEEK_NUMBER_ID', 'WEEK_NUMBER_NAME', '', array('SELECT_ADD_FOR_ORDER'=>array('WEEK_NUMBER_ID'), 'ORDER'=>'ORDER BY ADD_SELECT_1'));
        $c->setDescription($g['objMTS']->getSomeMessage('ITABASEH-MNU-307015'));//エクセル・ヘッダでの説明
        $c->setOutputType('update_table', new OutputType(new ReqTabHFmt(), new TextHiddenInputTabBFmt(''))); //テキストと、隠しテキストインプットを出力する
        ...
        //曜日
        $c = new IDColumn('PATTERN_DAY_OF_WEEK', $g['objMTS']->getSomeMessage('ITABASEH-MNU-307016'), 'B_DAY_OF_WEEK', 'DAY_OF_WEEK_ID', 'DAY_OF_WEEK_NAME', '', array('SELECT_ADD_FOR_ORDER'=>array('DAY_OF_WEEK_ID'), 'ORDER'=>'ORDER BY ADD_SELECT_1'));
        $c->setDescription($g['objMTS']->getSomeMessage('ITABASEH-MNU-307016'));//エクセル・ヘッダでの説明
        $c->setOutputType('update_table', new OutputType(new ReqTabHFmt(), new TextHiddenInputTabBFmt(''))); //テキストと、隠しテキストインプットを出力する
        ...
        //日
        $c = new TextColumn('PATTERN_DAY', $g['objMTS']->getSomeMessage('ITABASEH-MNU-307017'));
        $c->setDescription($g['objMTS']->getSomeMessage('ITABASEH-MNU-307017'));//エクセル・ヘッダでの説明
        ...
        //時間
        $c = new TextColumn('PATTERN_TIME', $g['objMTS']->getSomeMessage('ITABASEH-MNU-307018'));
        $c->setDescription($g['objMTS']->getSomeMessage('ITABASEH-MNU-307018'));//エクセル・ヘッダでの説明
    ...
    //実行停止期間
    $cg = new ColumnGroup($g['objMTS']->getSomeMessage('ITABASEH-MNU-307019'));
        //実行停止開始日付
        $c = new DateTimeColumn('EXECUTION_STOP_START_DATE', $g['objMTS']->getSomeMessage('ITABASEH-MNU-307020'), 'DATETIME', 'DATETIME', false);
        $c->setDescription($g['objMTS']->getSomeMessage('ITABASEH-MNU-307020'));//エクセル・ヘッダでの説明
        $c->setSecondsInputOnIU(false); //UI表示時に秒を非表示
        $c->setSecondsInputOnFilter(false); //フィルタ表示時に秒を非表示
        $c->setOutputType('update_table', new OutputType(new ReqTabHFmt(), new TextHiddenInputTabBFmt(''))); //テキストと、隠しテキストインプットを出力する
        ...
        //実行停止終了日付
        $c = new DateTimeColumn('EXECUTION_STOP_END_DATE', $g['objMTS']->getSomeMessage('ITABASEH-MNU-307021'), 'DATETIME', 'DATETIME', false);
        $c->setDescription($g['objMTS']->getSomeMessage('ITABASEH-MNU-307021'));//エクセル・ヘッダでの説明
        $c->setSecondsInputOnIU(false); //UI表示時に秒を非表示
        $c->setSecondsInputOnFilter(false); //フィルタ表示時に秒を非表示
        $c->setOutputType('update_table', new OutputType(new ReqTabHFmt(), new TextHiddenInputTabBFmt(''))); //テキストと、隠しテキストインプットを出力する
