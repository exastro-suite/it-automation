...
$tmpFx = function (&$aryVariant=array(),&$arySetting=array()){
    ...
    $arrayWebSetting['page_info'] = $g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-207080");
/*
Ansible（Legacy）作業パターン詳細
*/
    $tmpAry = array(
    ...
    $table = new TableControlAgent('B_ANSIBLE_LNS_PATTERN_LINK','LINK_ID',$g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-207090"), 'B_ANSIBLE_LNS_PATTERN_LINK_JNL', $tmpAry);
    $tmpAryColumn = $table->getColumns();
    $tmpAryColumn['LINK_ID']->setSequenceID('B_ANSIBLE_LNS_PATTERN_LINK_RIC');
    $tmpAryColumn['JOURNAL_SEQ_NO']->setSequenceID('B_ANSIBLE_LNS_PATTERN_LINK_JSQ');
    unset($tmpAryColumn);

    // QMファイル名プレフィックス
    $table->setDBMainTableLabel($g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-208010"));
    // エクセルのシート名
    ...
    $c = new IDColumn('PLAYBOOK_MATTER_ID',$g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-208050"),'D_ANSIBLE_LNS_PLAYBOOK','PLAYBOOK_MATTER_ID','PLAYBOOK_MATTER_NAME','');
    ...
    $c->setJournalDispIDOfMaster('PLAYBOOK_MATTER_NAME');
    $c->setRequired(true);//登録/更新時には、入力必須
    $table->addColumn($c);

    $c = new NumColumn('INCLUDE_SEQ',$g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-208070"));
    $c->setDescription($g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-208080"));//エクセル・ヘッダでの説明
