...
$tmpFx = function (&$aryVariant=array(),&$arySetting=array()){
    ...
    $arrayWebSetting['page_info'] = $g['objMTS']->getSomeMessage("ITAOPENST-MNU-110010");
    ...
    $table = new TableControlAgent('E_OPENST_PATTERN','PATTERN_ID',$g['objMTS']->getSomeMessage("ITAOPENST-MNU-110000"), 'E_OPENST_PATTERN_JNL', $tmpAry);
    ...
    // QMファイル名プレフィックス
    $table->setDBMainTableLabel($g['objMTS']->getSomeMessage("ITAOPENST-MNU-110020"));
    // エクセルのシート名
    $table->getFormatter('excel')->setGeneValue('sheetNameForEditByFile',$g['objMTS']->getSomeMessage("ITAOPENST-MNU-110020"));
    ...
    $c = new TextColumn('PATTERN_NAME',$g['objMTS']->getSomeMessage("ITAOPENST-MNU-110030"));
    $c->setDescription($g['objMTS']->getSomeMessage("ITAOPENST-MNU-110040"));//エクセル・ヘッダでの説明
    ...
    $tmpObjFunction = function($objColumn, $strEventKey, &$exeQueryData, &$reqOrgData=array(), &$aryVariant=array()){
        ...
        if( $modeValue=="DTUP_singleRecRegister" || $modeValue=="DTUP_singleRecUpdate" ){
            $exeQueryData[$objColumn->getID()] = 9; //OpenStack
    ...
    //オーケストレータリレーション
    $c = new IDColumn('ITA_EXT_STM_ID',$g['objMTS']->getSomeMessage("ITAOPENST-MNU-110050"),'B_ITA_EXT_STM_MASTER','ITA_EXT_STM_ID','ITA_EXT_STM_NAME','B_ITA_EXT_STM_MASTER');
    $c->setDescription($g['objMTS']->getSomeMessage("ITAOPENST-MNU-110060"));//エクセル・ヘッダでの説明
    $c->setHiddenMainTableColumn(true);//コンテンツのソースがヴューの場合、登録/更新の対象とする際に、trueとすること。setDBColumn(true)であることも必要。
    ...
    if( $tmpKeyExists===true ){
        if( $strTmpValue=="insConstruct" ){
            ...
            $objRadioColumn->setColLabel($g['objMTS']->getSomeMessage("ITAOPENST-MNU-110110"));
