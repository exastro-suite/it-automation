...
$tmpFx = function (&$aryVariant=array(),&$arySetting=array()){
    ...
    $arrayWebSetting['page_info'] = $g['objMTS']->getSomeMessage("ITAMATERIAL-MNU-102001");
    ...
    $table = new TableControlAgent('F_MATERIAL_LINKAGE_OPENSTACK','ROW_ID', $g['objMTS']->getSomeMessage("ITAMATERIAL-MNU-102002"), 'F_MATERIAL_LINKAGE_OPENSTACK_JNL', $tmpAry);
    $tmpAryColumn = $table->getColumns();
    $tmpAryColumn['ROW_ID']->setSequenceID('F_MATERIAL_LINKAGE_OPENSTACK_RIC');
    $tmpAryColumn['JOURNAL_SEQ_NO']->setSequenceID('F_MATERIAL_LINKAGE_OPENSTACK_JSQ');
    unset($tmpAryColumn);
    ...
    // QMファイル名プレフィックス
    $table->setDBMainTableLabel($g['objMTS']->getSomeMessage("ITAMATERIAL-MNU-102003"));
    // エクセルのシート名
    $table->getFormatter('excel')->setGeneValue('sheetNameForEditByFile', $g['objMTS']->getSomeMessage("ITAMATERIAL-MNU-102004"));
    ...
    $c = new TextColumn('MATERIAL_LINK_NAME',$g['objMTS']->getSomeMessage("ITAMATERIAL-MNU-102005"));
    $c->setDescription($g['objMTS']->getSomeMessage("ITAMATERIAL-MNU-102006"));//エクセル・ヘッダでの説明
    ...
    $objVldt = new LinkNameValidator2(1,128,false);//登録済みの紐付け先資材名であっても紐付け先(OpenStack)が違えば登録可能
    $c->setValidator($objVldt);
    $c->setRequired(true);//登録/更新時には、入力必須
    $table->addColumn($c);

    $c = new IDColumn('FILE_ID',$g['objMTS']->getSomeMessage("ITAMATERIAL-MNU-102007"),'G_FILE_MASTER','FILE_ID','FILE_NAME_FULLPATH','');
    $c->setDescription($g['objMTS']->getSomeMessage("ITAMATERIAL-MNU-102008"));//エクセル・ヘッダでの説明
    $c->setRequired(true);//登録/更新時には、入力必須
    $c->setEvent('update_table', 'onchange', 'fileId_upd');
    $c->setEvent('register_table', 'onchange', 'fileId_reg');
    $table->addColumn($c);

    $c = new IDColumn('CLOSE_REVISION_ID',$g['objMTS']->getSomeMessage("ITAMATERIAL-MNU-102009"),'G_FILE_MANAGEMENT_NEWEST','FILE_M_ID','CLOSE_REVISION','');
    $c->setDescription($g['objMTS']->getSomeMessage("ITAMATERIAL-MNU-102010"));//エクセル・ヘッダでの説明
    ...
    $strSetInnerText = $g['objMTS']->getSomeMessage("ITAMATERIAL-MNU-102011");
    ...
    $cg1 = new ColumnGroup($g['objMTS']->getSomeMessage("ITAMATERIAL-MNU-102012"));

        $c = new IDColumn('OPENST_TEMPLATE_CHK',$g['objMTS']->getSomeMessage("ITAMATERIAL-MNU-102013"),'D_FLAG_LIST_01','FLAG_ID','FLAG_NAME','');
        $c->setDescription($g['objMTS']->getSomeMessage("ITAMATERIAL-MNU-102014"));//エクセル・ヘッダでの説明
        $objVldt = new LinkageCheckValidator2($c);
        $c->setValidator($objVldt);
        $cg1->addColumn($c);

        $c = new IDColumn('OPENST_ENVIRONMENT_CHK',$g['objMTS']->getSomeMessage("ITAMATERIAL-MNU-102015"),'D_FLAG_LIST_01','FLAG_ID','FLAG_NAME','');
        $c->setDescription($g['objMTS']->getSomeMessage("ITAMATERIAL-MNU-102016"));//エクセル・ヘッダでの説明
        $objVldt = new LinkageCheckValidator2($c);
        $c->setValidator($objVldt);
        $cg1->addColumn($c);
