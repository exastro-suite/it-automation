...
$tmpFx = function (&$aryVariant=array(),&$arySetting=array()){
    ...
    $arrayWebSetting['page_info'] = $g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-206020");
/*
...
*/
    ...
    $table = new TableControlAgent('E_ANSIBLE_LNS_PATTERN','PATTERN_ID',$g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-206030"), 'E_ANSIBLE_LNS_PATTERN_JNL', $tmpAry);
    ...
    // エクセルのシート名
    $table->getFormatter('excel')->setGeneValue('sheetNameForEditByFile',$g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-206050"));
    ...
    $c = new TextColumn('PATTERN_NAME',$g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-206060"));
    $c->setDescription($g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-206070"));//エクセル・ヘッダでの説明
    ...
    $c = new IDColumn('ITA_EXT_STM_ID',$g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-206080"),'B_ITA_EXT_STM_MASTER','ITA_EXT_STM_ID','ITA_EXT_STM_NAME','B_ITA_EXT_STM_MASTER');
    $c->setDescription('プルダウン入力');//エクセル・ヘッダでの説明
    $c->setDescription($g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-206090"));//エクセル・ヘッダでの説明
    $c->setHiddenMainTableColumn(true);//コンテンツのソースがヴューの場合、登録/更新の対象とする際に、trueとすること。setDBColumn(true)であることも必要。
    ...
    $c = new NumColumn('TIME_LIMIT',$g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-207010"));
    $c->setDescription($g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-207020"));//エクセル・ヘッダでの説明
    $c->setHiddenMainTableColumn(true);//コンテンツのソースがヴューの場合、登録/更新の対象とする際に、trueとすること。setDBColumn(true)であることも必要。
    $c->setSubtotalFlag(false);
	$c->setValidator(new IntNumValidator(null,null));
    $table->addColumn($c);

    $cg = new ColumnGroup( $g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-207025") );

        $c = new IDColumn('ANS_HOST_DESIGNATE_TYPE_ID',$g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-207030"),'B_HOST_DESIGNATE_TYPE_LIST','HOST_DESIGNATE_TYPE_ID','HOST_DESIGNATE_TYPE_NAME','');
        $c->setDescription($g['objMTS']->getSomeMessage("ITAANSIBLEH-MNU-207040"));//エクセル・ヘッダでの説明
        $c->setHiddenMainTableColumn(true);//コンテンツのソースがヴューの場合、登録/更新の対象とする際に、trueとすること。setDBColumn(true)であることも必要。
