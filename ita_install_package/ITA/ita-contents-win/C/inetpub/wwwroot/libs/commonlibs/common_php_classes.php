...
class MessageTemplateStorage{
    ...
    public function getTemplateFilePath($strProductTypeId,$strCollectionGroupId,$strPostFix=""){
        return $this->getTemplateDirPath().DIRECTORY_SEPARATOR.$this->getLanguageFullVersion()."_".$strProductTypeId."_".$strCollectionGroupId.$strPostFix.".php";
    }
...
class DBConnectAgent{
    ...
    function getQueryTime($formatMode=0){
        ...
        if( $this->varQueryTimeMicro!==null && $this->varQueryTimeBase!==null){
            if($formatMode == 0){
                $tmpSec = date("s",$this->varQueryTimeBase) + $this->varQueryTimeMicro;
                $arySec = explode(".", $tmpSec);
                $sec = $arySec[0].".".sprintf('%06d', $arySec[1]);
                $ret = date("Y/m/d H:i:", $this->varQueryTimeBase).$sec;
            }else{
